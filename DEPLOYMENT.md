...
- Delivered `H99`/`H103`/`H104`/`H105` exception: two narrow internal allowlist families, `codex_separator_info` and `codex_separator_section`, may now be delivered for Codex sessions even while generic outbound remains hard-disabled. The first path stays entry-level separator anchored and now tolerates only tiny redraw-tail contamination on an otherwise clean separator entry plus a slightly wider bounded anchor-to-info gap; the second path strips prompt/footer/background-terminal chrome from mixed entries and assembles bounded separator-anchored narrative sections with subsection labels and indented list items. Both paths open a new Telegram post for each new block identity and reserve edits only for the same block identity instead of reopening broad line-by-line Telegram status delivery.
...
- `/health.messaging.allowlistDeliveryActive` and `/health.messaging.allowlistDeliveryScopes` expose whether narrow internal outbound allowlist paths such as `codex_separator_info` and `codex_separator_section` are active even while generic `deliveryEnabled` remains false
...
- `/health.messaging.adapters[0]` also exposes `allowlistDeliveryActive` and `allowlistDeliveryScopes`, so the narrow `H99`/`H105` Codex-only outbound exceptions can be distinguished from generic delivery state without digging through traces
...
- The shipped post-hard-break outbound path is still intentionally narrow: only `codex_separator_info` and `codex_separator_section` are allowed through the internal allowlist, where `codex_separator_info` covers simple separator-anchored `• info` blocks with at most one immediate continuation line and `codex_separator_section` covers chrome-stripped separator-anchored narrative sections with subsection labels and list items
