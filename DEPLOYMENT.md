...
- Messaging-specific note: the shipped messaging runtime is now transport-only. `messaging.event.trace` therefore reflects adapter startup, command publication, inbound handling, target provisioning, and explicit-delivery plumbing rather than automatic terminal-output interpretation.
...
- Stream-analysis note: session-stream capture remains an offline analysis aid only. It is no longer part of a live automatic messaging path.
...
- Automatic terminal-output-to-message delivery is not currently shipped. The runtime is transport-only and does not generate outbound Telegram messages from PTY output by itself.
- Explicit adapter-visible delivery still exists for inbound confirmations/callbacks and any future runtime seam that deliberately constructs a `MessageIntent`.
...
- `/health.messaging.mode` is `transport_only`
- `/health.messaging.boundaryContracts` is `["DeliveryAdapter","MessageIntent"]`
- `/health.messaging.deliveryEnabled` reflects whether configured adapters may deliver explicit transport events or explicit `MessageIntent` deliveries
- `/ready` returns the same `messaging` summary
- `/health.messaging.trace` and `/ready.messaging.trace` expose a bounded recent transport/runtime trace ring
...
- Automatic outbound status or summary delivery from terminal output is not part of the shipped runbook anymore; if such behavior is ever reintroduced, it will return as a separately approved future attempt rather than as an implicit hidden baseline
