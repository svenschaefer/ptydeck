...
- The first post-hard-break outbound family is now intentionally narrow: only `codex_separator_info` is allowed through the shipped allowlist path, and only when a Codex session emits a clean major-separator entry followed by a bounded clean `•` info block with at most one immediate continuation line
