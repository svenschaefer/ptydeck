...
- Delivered `H99`/`H103`/`H104` exception: one narrow internal allowlist family, `codex_separator_info`, may now be delivered for Codex sessions even while generic outbound remains hard-disabled. That path is entry-level separator anchored, now tolerates only tiny redraw-tail contamination on an otherwise clean separator entry plus a slightly wider bounded anchor-to-info gap, opens a new Telegram post for each new separator-anchored info block, and reserves edits only for the same block identity instead of reopening broad line-by-line Telegram status delivery.
...
- `/health.messaging.allowlistDeliveryActive` and `/health.messaging.allowlistDeliveryScopes` expose whether a narrow internal outbound allowlist path such as `codex_separator_info` is active even while generic `deliveryEnabled` remains false
...
- `/health.messaging.adapters[0]` also exposes `allowlistDeliveryActive` and `allowlistDeliveryScopes`, so the narrow `H99` Codex-only outbound exception can be distinguished from generic delivery state without digging through traces
...
- The first post-hard-break outbound family is now intentionally narrow: only `codex_separator_info` is allowed through the shipped allowlist path, and only when a Codex session emits a major-separator entry that is either clean or contaminated only by a tiny redraw-tail fragment, followed within the bounded horizon by a clean `•` info block with at most one immediate continuation line
