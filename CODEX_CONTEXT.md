...
Last updated: 2026-03-25 (H15 completed; v0.4.0-H1 completed; v0.4.0-H2 is now fully completed with `ARC-009` ... `ARC-012`; ARC-010 is fully completed with app command/UI facade extraction, bootstrap composition extraction, final app.js runtime-assembly extraction, and explicit closeout regression coverage after prior terminal runtime, resize orchestration, runtime event routing, grid render orchestration, WS runtime, command composer runtime, auth/bootstrap runtime, terminal-search runtime, deck runtime, session runtime, app lifecycle, layout runtime, settings state, command-target orchestration, shared app runtime-state orchestration, session UI facade orchestration, app layout/deck facade orchestration, and app session/runtime facade orchestration extractions; ARC-011 is completed by replacing the last UI-facing stream/runtime shortcuts with explicit session-runtime contracts for session mount/dispose lifecycle; ARC-012 is completed by adding architecture boundary regression coverage for stream/interpretation/state/UI modules and composed websocket-stream-store flow; full local quality gate green; roadmap versioning contract refreshed for active v0.4.x waves; docs-vs-outlook alignment synced; governance audit reconfirmed clean TODO/ROADMAP separation with explicit ARC-010 closeout subgoals; post-closeout governance sync reconfirmed the current open-task set `QLT-123` and `QLT-124` with `v0.4.0-H3` now active; session-artifact overlay hidden-state hardening completed; activity-status priority and split-chunk hardening completed)
Owner: `CODY`
Documentation sync status: all repository markdown files reviewed and aligned on 2026-03-25, including `docs/` review-note consolidation, TODO/ROADMAP separation validation, explicit open-task ownership sync, completion sync for `QLT-095`/`QLT-096`, `QLT-097`/`QLT-098`, `QLT-099`, `QLT-100`, `QLT-101`/`QLT-104`/`QLT-105`/`QLT-106`/`QLT-107`, hidden-scroll recovery closure for `QLT-108`/`QLT-109`, `QLT-110`/`QLT-111` terminal search completion, completion sync for the declarative autocomplete milestone (`QLT-112` ... `QLT-115`), completion sync for runtime metadata event consistency (`QLT-116` ... `QLT-118`), closure of the runtime store and contract hardening block (`QLT-119` ... `QLT-122`), completion of the explicit lifecycle-state-model block (`LIF-001` ... `LIF-006`), promotion of the stream-interpretation/plugin follow-up into `TODO.md` / `ROADMAP.md` as `v0.3.0-H13`, completion of `ARC-003` stream-adapter foundation, completion of `ARC-004` deterministic plugin-engine registry foundation, completion of `ARC-005` interpretation-action contract plus dispatcher integration, completion of `ARC-006` built-in stream-interpreter baseline, completion of `ARC-007` artifact-oriented interpretation and session-surface rendering, completion of `ARC-008` regression hardening for the full stream-interpretation foundation, promotion and completion sync for the browser-notification follow-up `v0.3.0-H14` (`LIF-007` ... `LIF-009`), completion of the auth hardening follow-up `v0.3.0-H15` (`ENT-026` ... `ENT-028`) including one-time WS handshake tickets and explicit `AUTH_MODE`, send-body newline normalization hardening so only the configured final terminator emits control-submit sequences, explicit decomposition of deferred plugin/stream-interpretation architecture into implementable `ARC-003` ... `ARC-008` subtasks, the `TODO-OUTLOOK.md` filename correction, docs-derived deferred stack-evolution capture in `TODO-OUTLOOK.md` (`ALT-001` ... `ALT-006`) alongside deferred `REM-001` ... `REM-009`, promotion of the next active block from `TODO-OUTLOOK.md` into `TODO.md` / `ROADMAP.md` as `v0.4.0-H1` (`OBS-001` ... `OBS-004`), completion sync for `v0.4.0-H1` (`OBS-001` ... `OBS-004`) across code/tests/docs, promotion of remaining layered-frontend-architecture tasks from `docs/Codebase Review - Decomposing app js into a Layered Frontend Architecture.md` into active backlog as `v0.4.0-H2` (`ARC-009` ... `ARC-012`), explicit split of `ARC-010` into closeout subgoals `ARC-010A` ... `ARC-010D`, completion sync for the recent `ARC-010` controller-extraction increments (`session-terminal-runtime`, `session-terminal-resize`, `runtime-event`, `session-grid`, `ws-runtime`, `command-composer-runtime`, `auth-bootstrap-runtime`, `terminal-search-runtime`, `deck-runtime`, `session-runtime`, `layout-runtime`, `app-lifecycle`, `session-settings-state`, `command-target-runtime`, `app-runtime-state`, `session-ui-facade`, `app-layout-deck-facade`, `app-session-runtime-facade`, `app-command-ui-facade`, `app-bootstrap-composition-controller`, `app-runtime-composition-controller`, `app-architecture-closeout`), session-artifact overlay UX synchronization (artifacts no longer consume terminal height and can be dismissed until new artifact content arrives), follow-up overlay hardening so the dismiss control stays hidden unless the artifact overlay itself is visible, activity-status priority hardening so richer timer/progress patterns outrank lower-detail matches, split-chunk redraw reconstruction for activity-status parsing so status upgrades remain stable across partial terminal updates, roadmap versioning clarification for active `v0.4.x` waves, full local quality-gate confirmation for the latest ARC-010 increment, status clarification that `H2` is now the active wave, `ARC-010A` closeout sync across planning docs, `ARC-010B` closeout sync across planning docs, `ARC-010C` closeout sync across planning docs, `ARC-010D` closeout sync across planning docs, and a final governance audit confirming no TODO/ROADMAP drift after the latest cycle.
...
- `TODO.md` currently contains open tasks only (`QLT-123`, `QLT-124`); completed history remains exclusively in `DONE.md`.
