...
- The xterm render layer must not shadow the native scrollbar gutter. The current delivered contract keeps `.terminal-mount .xterm-viewport` as the pointer target, leaves pointer/focus behavior enabled on `.xterm-screen`, and disables pointer events only on `.xterm-screen canvas` so native scrollbar dragging can hit the real viewport without breaking terminal focus and context-menu behavior.
