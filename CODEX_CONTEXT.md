...
- External terminal/SSH tool survey follow-up remains intentionally tracked as explicit backlog/wave work: the earlier remote-session/auth/trust/reconnect and saved-profile slices (`REM-001` ... `REM-005`) are complete, `REM-008A` ... `REM-008C` are now active for theme import/export compatibility, and the remaining deferred backlog stays limited to controlled file transfer (`REM-006`) plus sharing/read-only mode (`REM-007`).
