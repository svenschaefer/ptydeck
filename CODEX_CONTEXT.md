...
Last updated: 2026-03-25 (H15 completed; v0.4.0-H1 completed; ARC-009 completed in active v0.4.0-H2; ARC-010 is fully completed with app command/UI facade extraction, bootstrap composition extraction, final app.js runtime-assembly extraction, and explicit closeout regression coverage after prior terminal runtime, resize orchestration, runtime event routing, grid render orchestration, WS runtime, command composer runtime, auth/bootstrap runtime, terminal-search runtime, deck runtime, session runtime, app lifecycle, layout runtime, settings state, command-target orchestration, shared app runtime-state orchestration, session UI facade orchestration, app layout/deck facade orchestration, and app session/runtime facade orchestration extractions; ARC-011 is now completed by replacing the last UI-facing stream/runtime shortcuts with explicit session-runtime contracts for session mount/dispose lifecycle; full local quality gate green; roadmap versioning contract refreshed for active v0.4.x waves; docs-vs-outlook alignment synced; governance audit reconfirmed clean TODO/ROADMAP separation with explicit ARC-010 closeout subgoals; post-closeout governance sync reconfirmed the current open-task set `ARC-012`, `QLT-123`, and `QLT-124` with unchanged roadmap sequencing; session-artifact overlay hidden-state hardening completed; activity-status priority and split-chunk hardening completed)
Owner: `CODY`
...
- `TODO.md` currently contains open tasks only (`ARC-012`, `QLT-123`, `QLT-124`); completed history remains exclusively in `DONE.md`.
