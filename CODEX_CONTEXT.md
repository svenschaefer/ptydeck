...
- `TODO.md`: active promoted tasks are `QLT-292` and `QLT-293`.
- `ROADMAP.md`: active wave `v0.4.0-H171` is promoted; no queued wave is currently promoted.
- `QLT-290`, `QLT-291`, and `QLT-294` are complete as the delivered slices inside `v0.4.0-H171`.
...
The latest validation on 2026-05-05 after the `QLT-294` closeout reran `npm --prefix frontend run test:coverage`, `npm --prefix backend run test:coverage`, `npm run docs:check`, `npm run lint`, `npm run test`, `npm run test:coverage:check`, and `git diff --check`. The validated coverage totals are root tooling `92.82%` line / `77.09%` branch, backend `95.59%` line / `89.35%` branch, and frontend `97.38%` line / `89.89%` branch.
