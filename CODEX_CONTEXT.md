...
Last updated: 2026-05-07 (`v0.4.0-H175` is now fully closed on `main`, leaving no active or queued wave in `TODO.md` / `ROADMAP.md`; fresh repo-wide quality evidence is current for root, backend, and frontend coverage lanes. The backend runtime now also delegates retained custom-command catalog, deck catalog, session-control fallback-owner/state shaping, and persisted runtime snapshot serialization into `backend/src/runtime-catalog-authority.js` in addition to session-control client/session authority in `backend/src/runtime-session-control-authority.js`, share/spectator request-access and metrics helpers in `backend/src/runtime-access-policy.js` and `backend/src/runtime-metrics.js`, deck/session-state shaping in `backend/src/runtime-session-state.js`, persisted startup/restore authority in `backend/src/runtime-startup-restore.js`, share/layout/connection/workspace authority in `backend/src/runtime-library-authority.js`, retained catalog/state normalization in `backend/src/runtime-library-normalization.js`, and SSH trust lifecycle handling in `backend/src/runtime-ssh-trust.js`; retained backend normalization and control-authority regressions now explicitly cover strict layout/workspace guard rails, deck-share defaults, persisted share-link revoke metadata, attachment header-array parsing, inactive/spectator attachment rejection, implicit auto-assign resets, messaging-owner access, scope validation, and owner-preserving control-state broadcast fallbacks; the backend session manager now also delegates create/restart/close/guardrail orchestration into `backend/src/session-manager-session-runtime.js` in addition to cleanup in `backend/src/session-manager-cleanup.js`, SSH remote-runtime state shaping in `backend/src/session-manager-remote-runtime.js`, startup/post-start input lifecycle in `backend/src/session-manager-startup-runtime.js`, create/update/restart lifecycle state shaping and replay retention in `backend/src/session-manager-lifecycle.js`, app-identity refresh and output-heuristic arbitration in `backend/src/session-manager-app-identity-runtime.js`, and SSH launch/reconnect orchestration in `backend/src/session-manager-launch-runtime.js`, while the server-authoritative quick-send usage helper is covered directly in `backend/test/session-quick-send-usage.test.js`; retained session-manager lifecycle regressions now explicitly cover local-session default shaping, quick-send usage normalization, theme-slot fallback behavior, noop identity application, unavailable foreground refreshes, and null-session terminal-signal handling; trusted-local handoff still resolves runtime client id through the session-control authority seam and fails closed on stale session takeover targets; SSH launches still pin `HostKeyAlgorithms` to the trusted host-key types for the selected target, pass an absolute managed `UserKnownHostsFile` path into spawned `ssh` processes, preserve canonical padded host-key base64 so the rendered managed `ssh_known_hosts` file stays parseable by OpenSSH, keep secret-backed auth on one masked action-dialog seam, expose slash-command host-key lifecycle management through `/ssh hostkey ...`, surface first-connect and rotation trust guidance directly in `Connections`, and extend one-shot `/ssh ...` parity with `--deck`, `--cwd`, and `--command`; multiline terminal sends still distinguish shell-family sessions from coding-agent/unknown sessions so the frontend can normalize internal line separators to the configured terminator for shell-targeted `Send`, custom-command, and quick-send flows instead of always forcing raw `LF` inside multiline payloads; the frontend runtime composition now also delegates initialization/error/reclaim state plus session-access composition into `frontend/src/public/app-runtime-initialization-access-composition.js`, deck/session-grid action orchestration into `frontend/src/public/app-runtime-session-grid-actions.js`, retained session/grid/operator surface wiring into `frontend/src/public/app-runtime-session-surface-assembly.js`, and the existing trusted-local handoff/layout, bootstrap wiring, runtime-event recovery, operator-support assembly, and startup/workflow/palette/initialization seams out of `frontend/src/public/app-runtime-composition-controller.js`; `frontend/src/public/command-executor.js` now also delegates retained runtime/router helpers into `frontend/src/public/command-executor-runtime-router.js`; `frontend/src/public/connection-profile-runtime-controller.js` now also delegates saved-profile selection, draft/trust presentation state, guarded operator fallbacks, and retained presentation/lifecycle/action assembly into `frontend/src/public/connection-profile-runtime-presentation.js` and `frontend/src/public/connection-profile-runtime-assembly.js`; `frontend/src/public/workspace-preset-runtime-controller.js` now also delegates workspace snapshot/group/layout orchestration into `frontend/src/public/workspace-preset-runtime-state.js`; and retained split-layout/layout-profile state shaping now flows through `frontend/src/public/split-layout-state.js` instead of duplicated helper logic in both layout controllers. Retained operator-interaction regressions now explicitly cover workspace preset CRUD selector fallbacks, grouped deck filtering, stream-interpretation invalid registration and malformed batch fail-closed behavior, and deterministic deck-sidebar session activation waits inside `frontend/test/app.test.js`.)
...
- backend coverage: `96.79%` line / `90.17%` branch / `88.88%` funcs
- frontend coverage: `97.52%` line / `90.55%` branch / `82.44%` funcs

Current lane health:

- `npm --prefix backend run test` is green on the current tree.
- `npm --prefix frontend run test` is green on the current tree.
- `npm --prefix frontend run test:coverage` is green on the current tree.

Current highest retained hotspots:

- backend:
  - `backend/src/runtime.js`: `2788` lines, `84.43%` line / `76.86%` branch
  - `backend/src/runtime-library-normalization.js`: `1724` lines, `85.79%` line / `80.27%` branch
  - `backend/src/session-manager.js`: `1121` lines, `95.09%` line / `75.48%` branch
  - `backend/src/runtime-session-control-authority.js`: `445` lines, `95.06%` line / `89.86%` branch
  - `backend/src/runtime-catalog-authority.js`: `406` lines, `95.57%` line / `90.53%` branch
  - `backend/src/session-manager-session-runtime.js`: `238` lines, `100.00%` line / `65.63%` branch
- frontend:
  - `frontend/src/public/app-runtime-composition-controller.js`: `1617` lines, still the largest retained frontend runtime-composition monolith
  - `frontend/src/public/workspace-preset-runtime-state.js`: `980` lines, `95.51%` line / `82.93%` branch
  - `frontend/src/public/split-layout-runtime-controller.js`: `895` lines, `91.71%` line / `75.00%` branch
  - `frontend/src/public/ui/session-terminal-runtime-controller.js`: `824` lines, `92.35%` line / `85.23%` branch
  - `frontend/src/public/layout-profile-runtime-controller.js`: `631` lines, `93.32%` line / `86.28%` branch
  - `frontend/src/public/stream-interpretation-plugin-engine.js`: `212` lines, `95.75%` line / `90.22%` branch

Completed on this review wave:

- `QLT-318` Owner `FE`: extracted retained session/grid/operator surface wiring from `frontend/src/public/app-runtime-composition-controller.js` into `frontend/src/public/app-runtime-session-surface-assembly.js` and closed it with direct deterministic regressions
- `QLT-319` Owner `FE`: isolated shared split-layout/layout-profile orchestration helpers into `frontend/src/public/split-layout-state.js`
- `QLT-320` Owner `FE`: hardened retained runtime/operator-interaction coverage across `frontend/src/public/workspace-preset-runtime-state.js`, `frontend/src/public/ui/session-terminal-runtime-controller.js`, and `frontend/src/public/stream-interpretation-plugin-engine.js`
