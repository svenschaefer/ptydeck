...
- The completed H38 remote-session foundation is intentionally backend-first: remote `ssh` session-kind support, auth handling, and host-key trust now all land on one persisted session contract before any later FE-only connection-profile, sharing, or transfer UX is layered on top.
