...
- Frontend send-safety UX now surfaces the active target directly beside the composer and routes risky sends through an inline confirmation banner with explicit reason labels and a one-shot override path instead of silently blocking or auto-sending; the completed H50 follow-up removed preset-based input-safety UX entirely, so current FE settings/help/command surfaces now expose only the explicit per-session option fields that already exist in the backend contract.
...
- The completed H49 workflow-source slice now adds one explicit `frontend/src/public/slash-workflow-source-adapter.js` layer: `status`, `summary`, `exit-code`, and `session-state` are read from store-backed session data and artifacts, `line` and `visible-line` are read from the mounted xterm buffer, terminal-backed sources fail explicitly when no terminal buffer is available, and normal terminal activity handling still avoids any plugin-style stream-interpretation revival.
...
- The completed H51 observability wave now adds deterministic distributed-trace linkage across REST, WebSocket, and PTY/session lifecycle flows: backend responses emit `x-ptydeck-trace-id` and `x-ptydeck-correlation-id`, WS snapshot/session payloads carry matching `trace` metadata, and debug-mode browsers can inspect bounded persisted trace records through `window.__PTYDECK_TRACE_DEBUG__`.
