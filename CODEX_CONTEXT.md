...
Last updated: 2026-03-24 (H15 completed; OBS-001 completed in active v0.4.0-H1; v0.4.0-H2 queued)
Owner: `CODY`
...
- Backend now exposes `GET /metrics` with Prometheus-style baseline metrics for HTTP traffic, request durations, active sessions, active WebSocket connections, explicit session lifecycle counters (`created`, `started`, `exited`, `unrestored`), per-lifecycle session gauges, WebSocket reconnect totals, and websocket error counters by reason.
