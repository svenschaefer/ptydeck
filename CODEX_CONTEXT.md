...
- The next queued release wave is `v0.4.0-H61`.
...
- The next send-history follow-up must harden that baseline into a safer end-user workflow: the UI still needs explicit local-persistence disclosure plus clear/delete controls, restore-to-input still needs draft-overwrite protection when the composer is non-empty, and the dialog should stay pinned to the session it was opened for instead of resetting search/selection state whenever the global active session changes underneath it.
