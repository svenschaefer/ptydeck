...
- External terminal/SSH tool survey follow-up is intentionally deferred to `TODO-OUTLOOK.md` as explicit medium-term backlog (`REM-001` ... `REM-013`) covering SSH session kinds, auth/trust, saved connection profiles, remote-session reconnect semantics, controlled file transfer, sharing/read-only mode, theme import/export compatibility, and `withfig/autocomplete`-inspired declarative/generator-backed command-completion architecture.
