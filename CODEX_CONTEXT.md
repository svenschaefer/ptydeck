...
Fresh repo-wide evidence on the current tree after the full `H177` closeout:

- root tooling coverage: `92.84%` line / `77.05%` branch / `94.83%` funcs
- backend coverage: `97.01%` line / `90.56%` branch / `88.68%` funcs
- frontend coverage: `97.63%` line / `90.73%` branch / `82.27%` funcs
...
- backend:
  - `backend/src/runtime.js`: `2441` lines, still the largest retained backend authority surface after the session-resource extraction
  - `backend/src/runtime-library-normalization.js`: `1724` lines, `87.12%` line / `82.73%` branch
  - `backend/src/session-manager.js`: `841` lines, `95.72%` line / `79.82%` branch
  - `backend/src/runtime-session-resource-authority.js`: `372` lines, direct seam coverage now lives in `backend/test/runtime-session-resource-authority.test.js`
  ...
  - `backend/src/runtime-session-state.js`: `275` lines, `99.03%` line / `96.15%` branch
- frontend:
  - `frontend/src/public/app-runtime-composition-controller.js`: `1333` lines, still the largest retained frontend runtime-composition monolith after the helper-assembly extraction
  - `frontend/src/public/workspace-preset-runtime-state.js`: `697` lines, `97.99%` line / `87.71%` branch
  - `frontend/src/public/ui/session-terminal-runtime-controller.js`: `824` lines, `92.96%` line / `86.33%` branch
  - `frontend/src/public/connection-profile-draft-state.js`: `680` lines, `97.35%` line / `81.41%` branch
  - `frontend/src/public/ws-runtime-controller.js`: `461` lines, `100.00%` line / `93.33%` branch
  - `frontend/src/public/split-layout-runtime-controller.js`: `657` lines, `95.43%` line / `77.93%` branch
  - `frontend/src/public/layout-profile-runtime-controller.js`: `504` lines, `93.06%` line / `85.80%` branch
...
Current active tasks after the `H177` closeout:
...
They remain intentionally deferred because the diagnostics are not live product/runtime authority paths and the messaging third-attempt chain still depends on separate deferred product decisions.
