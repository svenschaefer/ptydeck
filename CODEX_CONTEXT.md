...
Last updated: 2026-03-27 (`v0.4.0-H31` is now fully completed with `CMD-004`, `CMD-005`, and `CMD-006` closed for fuzzy and personalized command suggestions: composer autocomplete and the command palette now share one deterministic discovery-ranking baseline in `frontend/src/public/command-discovery-ranking.js`, exact-prefix matches remain authoritative ahead of broader fuzzy matches, and browser-local recency personalization is layered on top only for otherwise-comparable results without inventing a backend/user-account contract; `v0.4.0-H32` is now the active delivery wave for scoped custom-command sets with `CMD-007`, `CMD-008`, and `CMD-009`; `v0.4.0-H30` remains fully completed with `CMD-001`, `CMD-002`, and `CMD-003` closed for parameterized custom command templates; `v0.4.0-H29` remains fully completed with `UX-014`, `UX-015`, and `UX-016` closed for control-plane and execution-plane separation; `v0.4.0-H28` remains fully completed with `UX-011`, `UX-012`, and `UX-013` closed for the split-layout foundation; `v0.4.0-H27` remains fully completed with `UX-008`, `UX-009`, and `UX-010` closed for workspace-group broadcast input; slash help and overview output include the direct-route marker `@` alongside `>` and `/`, so the visible command surface is represented as `Commands: @ > / ...`; direct terminal typing stability hotfixes from the recent post-H24 incident remain in place, including the final deck-independent reorder guard in `frontend/src/public/ui/session-grid-controller.js`; persisted frontend stream debug tracing remains available in debug mode via `window.__PTYDECK_STREAM_DEBUG__`; full local quality gate green)
Owner: `CODY`
Documentation sync status: all repository markdown files reviewed and aligned on 2026-03-27, including the full `v0.4.0-H31` closeout sync for `CMD-004` through `CMD-006`, the `v0.4.0-H32` promotion sync for `CMD-007` through `CMD-009`, the earlier `v0.4.0-H30` closeout sync for `CMD-001` through `CMD-003`, the earlier `v0.4.0-H29` closeout sync for `UX-014` through `UX-016`, the earlier `v0.4.0-H28` closeout sync for `UX-011` through `UX-013`, the earlier `v0.4.0-H27` closeout sync for `UX-008` through `UX-010`, the earlier `v0.4.0-H26` closeout sync for `UX-005` through `UX-007`, the slash-help `@` overview correction, the earlier `v0.4.0-H25` closeout sync for `QLT-144` through `QLT-149`, the recent direct-terminal-typing hotfix sync in `frontend/src/public/ui/session-grid-controller.js`, TODO/ROADMAP separation validation, explicit open-task ownership validation, and persistent context capture for the current active-wave H32 roadmap state.
...
- The active H32 custom-command wave must keep scope/precedence authoritative in the backend contract first: `global`, `project`, and `session` command scopes, optional `sessionId` binding, and precedence metadata must be persisted and normalized server-side before the frontend extends slash help, autocomplete, preview, or command-palette behavior around them.
