...
On 2026-05-06, a fresh whole-repo quality and coverage review was rerun after the multiline shell-send transport fix so the next quality wave is based on the current tree instead of the earlier `H172` closeout snapshot. `QLT-302`, `QLT-303`, `QLT-304`, and `QLT-305` have since been completed as the first four delivered slices inside `H173`.
