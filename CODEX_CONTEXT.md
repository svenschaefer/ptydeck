...
Last updated: 2026-05-16 (the backend runtime still delegates startup/session-dispatch assembly into `backend/src/runtime-session-dispatch-assembly.js` plus retained route authority into `backend/src/runtime-session-dispatch-authority.js`, start/stop orchestration into `backend/src/runtime-lifecycle.js`, API session shaping and messaging/session authority into `backend/src/runtime-session-messaging-authority.js`, manager-session lifecycle-to-messaging/broadcast/warmup/persistence bridging into `backend/src/runtime-session-event-authority.js`, replay export/excerpt shaping plus session file-transfer guard rails and restore-create forwarding into `backend/src/runtime-session-resource-authority.js`, custom-command and deck catalog shaping into `backend/src/runtime-catalog-authority.js`, startup-readiness state into `backend/src/runtime-startup-readiness.js`, session-control client/session authority into `backend/src/runtime-session-control-authority.js`, share/spectator request-access and metrics helpers into `backend/src/runtime-access-policy.js` and `backend/src/runtime-metrics.js`, deck/session-state shaping into `backend/src/runtime-session-state.js`, persisted startup/restore authority into `backend/src/runtime-startup-restore.js`, operator-client composer placement persistence into `backend/src/runtime-operator-composer-authority.js`, share/layout/connection/workspace authority into `backend/src/runtime-library-authority.js`, retained catalog/state normalization into `backend/src/runtime-library-normalization.js`, and SSH trust lifecycle handling into `backend/src/runtime-ssh-trust.js`. The backend session manager still delegates retained runtime facade wiring into `backend/src/session-manager-runtime-facade.js` alongside constructor lifecycle/runtime wiring in `backend/src/session-manager-runtime-assembly.js`, PTY attachment and live stream handling in `backend/src/session-manager-pty-runtime.js`, mutation handling in `backend/src/session-manager-mutation-runtime.js`, trace/activity/input/resize/signal orchestration in `backend/src/session-manager-terminal-runtime.js`, trace-envelope and startup terminal-query helper shaping in `backend/src/session-manager-trace-runtime.js`, create/restart/close/guardrail orchestration in `backend/src/session-manager-session-runtime.js`, cleanup in `backend/src/session-manager-cleanup.js`, SSH remote-runtime state shaping in `backend/src/session-manager-remote-runtime.js`, startup/post-start input lifecycle in `backend/src/session-manager-startup-runtime.js`, create/update/restart lifecycle shaping in `backend/src/session-manager-lifecycle.js`, replay-output append/export/excerpt handling in `backend/src/session-manager-replay-runtime.js`, app-identity refresh and output-heuristic arbitration in `backend/src/session-manager-app-identity-runtime.js`, and SSH launch/reconnect orchestration in `backend/src/session-manager-launch-runtime.js`. Direct local PowerShell sessions remain first-class local launch aliases with explicit WSL fallback resolution for standard Windows PowerShell and PowerShell 7 install paths plus fail-closed launcher validation, trusted-local handoff still resolves runtime client id through the session-control authority seam and fails closed on stale session takeover targets, SSH launches still keep secret-backed auth on one masked action-dialog seam plus trusted-host-key pinning and absolute managed known-hosts handling, multiline shell sends still normalize internal separators to the configured terminator for shell-family sessions, the runtime create-session path now passes the freshly shaped API payload into the follow-up session update broadcast so ultra-fast guardrail cleanup cannot race a successful `POST /api/v1/sessions` into a late `404`, and the frontend runtime composition still delegates startup/reclaim/operator helper wiring into `frontend/src/public/app-runtime-startup-helper-composition.js` plus layout foundation assembly into `frontend/src/public/app-runtime-layout-foundation-assembly.js`, access-control composition into `frontend/src/public/app-runtime-access-control-composition.js`, stateful split-layout merge/mutation orchestration into `frontend/src/public/layout-split-layout-runtime-model.js`, and layout/workspace selection shaping into `frontend/src/public/layout-workspace-selection-state.js` alongside layout/workspace apply plus visible-session orchestration in `frontend/src/public/layout-workspace-orchestration-state.js`. The operator input composer now also supports server-persisted per-operator-client placement through `frontend/src/public/operator-composer-placement-runtime-controller.js`, which reuses the shared composer body for `shared-footer` versus `active-overlay` placement, renders non-resizing active-terminal overlays, manages per-terminal pinned overlay composers, and applies persisted placement updates from the runtime snapshot and targeted `composer-placement.updated` events. The frontend command-composer and app-runtime state layer now also clear stale global UI errors deterministically when a new send attempt or bounded blocked-send feedback supersedes an older error, and the composition helper seam now exposes connection-state control for deterministic runtime-composition/reconnect tests.)

## WebSocket Send Hardening Baseline

- Runtime WebSocket send failures must never abort the HTTP mutation that triggered the broadcast.
- `backend/src/runtime.js` now treats outbound WebSocket sends as best-effort side effects:
  - every broadcast path wraps `socket.send(JSON.stringify(...))` in a fail-closed guard
  - on synchronous send failure it records `recordWsError("socket_send_failed")`
  - it emits `logDebug("ws.send.failed", ...)`
  - it marks the socket with `closeReasonHint: "send_failed"`
  - it closes or terminates the stale socket instead of letting the throw escape back into the active REST request
- This hardening is specifically required for the operator composer-placement autosave path because `PATCH /api/v1/operator/composer-placement` runs in the foreground of normal typing and previously could collapse into an empty upstream reply / transient proxy `502` if one already-attached WebSocket was in a broken send state.
- `backend/src/runtime-ws-connection.js` now applies the same guarded send behavior to the initial `snapshot` payload and only records `ws.snapshot.sent` after a successful send, so accepted-socket bootstrap cannot falsely report a delivered snapshot after a synchronous send failure.
- The regression contract is now explicit in:
  - `backend/test/runtime-ws-connection.test.js` for snapshot send failure handling
  - `backend/test/ws.integration.test.js` for the integrated `PATCH /operator/composer-placement` path when a live `composer-placement.updated` WebSocket send throws

## Direct Local PowerShell Session Baseline

- Local session launches now support direct PowerShell aliases through the existing `/new [shell]` contract:
  - `/new ps` normalizes to `powershell.exe`
  - `/new powershell` normalizes to `powershell.exe`
  - `/new pwsh` normalizes to `pwsh.exe`
- The aliasing is intentionally backend-authoritative in `backend/src/session-launch-spec.js` so the frontend can keep `/new [shell]` as one generic contract instead of growing a separate launcher-specific request shape.
- The backend launch runtime now resolves those launchers before PTY spawn instead of relying only on the imported backend PATH:
  - `powershell.exe` falls back to the standard WSL-visible Windows PowerShell path
  - `pwsh.exe` falls back to the highest available `C:\Program Files\PowerShell\<version>\pwsh.exe` installation surfaced under `/mnt/c/Program Files/PowerShell/...`
- If the requested local PowerShell launcher still cannot be resolved, the backend now returns a fail-closed `ValidationError` with an operator-facing remediation hint instead of surfacing the raw PTY `execvp(3)` spawn failure.
- `backend/src/shell-adapter.js` now classifies `powershell`, `powershell.exe`, `pwsh`, and `pwsh.exe` as one explicit `powershell` shell family.
- Current PowerShell family behavior is intentionally conservative:
  - cwd tracking: unsupported
  - shell-block tracking: unsupported
...
- Backend restore semantics are now lifecycle-aware:
  ...
  - persisted exited sessions restore as exited without relaunching a PTY, while preserving `exitCode`, `exitSignal`, and `exitedAt`
...
- Unexpected PTY exit must no longer delete the persisted session record:
  - `backend/src/session-manager-launch-runtime.js` now leaves an unexpectedly exited session in the manager map with `meta.state: "exited"` instead of immediately deleting it
  - `backend/src/runtime-startup-restore.js`, `backend/src/runtime-session-resource-authority.js`, and `backend/src/session-manager-runtime-facade.js` must preserve `initialState: "exited"` plus exit metadata all the way through restore-create wrappers
  - this is specifically required so a hung or broken terminal that exits before backend restart still comes back as a visible `exited` card that can be relaunched from the header instead of silently disappearing after restore
...
- The header lifecycle toggle also has to stay consistent across both creation and update paths:
...
- Trusted-local session control now has one more local-dev transport invariant:
  - with bearer auth enabled, `frontend/src/public/ws-runtime-controller.js` must keep using the `/auth/ws-ticket` bootstrap so the browser can carry authenticated session-control identity through the WebSocket upgrade without exposing bearer material in the subprotocol
  - when that same authenticated WebSocket path is available but the current runtime still has no bearer token, the frontend must attempt `bootstrapDevAuthToken({ reason: "ws-missing-auth" })` inside the WebSocket protocols provider before falling back to trusted-local-only protocols; otherwise an auth-enabled browser session can get stuck in a load-time `connecting`/`reconnecting` loop after one transient bootstrap miss
  - with `AUTH_MODE=off`, that same runtime must not introduce an extra HTTP ticket fetch just to preserve trusted-local control; it now carries the stable trusted-local client id/label directly in a `ptydeck.client.<base64url-json>` WebSocket subprotocol instead
  - `backend/src/runtime-access-policy.js` now parses that trusted-local metadata protocol, and `backend/src/runtime-ws-upgrade.js` consumes it in auth-disabled mode to restore the same logical local device after a frontend-only refresh without drifting back to an ephemeral connection id
- The backend WebSocket accept path now has one additional stability invariant:
  - the newly accepted socket must not receive the immediate post-attach session-control refresh burst that exists to inform already-connected peer sockets about attachment/control-state changes
  - `backend/src/runtime-ws-connection.js` now prefers `broadcastSessionControlRefreshForAuthExceptSocket(...)` when the runtime provides it
  - `backend/src/runtime.js` now implements that helper by reconciling controller state as before but broadcasting the resulting `session.updated` refresh events only to peer sockets, explicitly excluding the socket that just received its authoritative `snapshot`
  - this rule exists because the new socket's snapshot already contains the reconciled session/control state, so replaying the same control-refresh burst back into that socket is redundant and was empirically shown to destabilize the connection on load
  - the concrete 2026-05-14 validation used a naked ticket-authenticated WebSocket probe against the live backend path: before the fix the socket reproducibly opened, received `snapshot` plus `16` immediate `session.updated` frames, and then aborted around `18 ms` later with `1006`; after excluding the new socket from that burst, the same probe remained stable for `15 s` until manual close while still allowing peer-socket refresh behavior
  - auth-disabled ws tickets remain only as a bounded compatibility path when an older client still presents them, and the bare `ptydeck.v1` protocol remains the final fallback when no trusted-local metadata is available at all
  - `frontend/src/public/runtime-event-controller.js` must not overwrite an already-established runtime/session-control client id from later runtime snapshots unless the snapshot agrees with the current client id; the stable trusted-local identity is authoritative after initialization, while blank or connection-scoped snapshot ids are not
  - `frontend/src/public/operator-composer-placement-runtime-controller.js` startup initialization must treat both the pre-auth `401 Missing bearer token` window and the pre-attach `409 OperatorClientRequired` window as bounded bootstrap races instead of fatal composer-placement initialization failures, because page load can reach that endpoint before auth and WebSocket attach have converged
...
- The current session-create/delete stabilization baseline now includes two additional frontend invariants:
  ...
  - session creation is now intentionally fail-closed on the frontend while the live connection state is anything other than explicit `connected`; the header `New Session` button and `/new` both refuse to create during `connecting` or `reconnecting`, because REST can succeed before the WebSocket-side session-control attachment exists and would otherwise create sessions that immediately trip the backend `active attached session client` guard
...
- The current reconnect stabilization baseline now includes one explicit frontend fail-closed rule:
  - `frontend/src/public/session-control-runtime-state.js` must treat any non-`connected` runtime `connectionState` as a bounded no-write window, disabling implicit-owner fallback, write access, take/release/transfer/manage affordances, and optimistic controller badges until the WebSocket/session-control attachment is live again
  - `frontend/src/public/session-control-runtime-controller.js` must carry that live `connectionState` through the session-control context, and `frontend/src/public/app-runtime-session-access-assembly.js` must source it from the store so the real shared composer, quick-send, resize, and session-control UI paths all inherit the same fail-closed reconnect contract
  - the operator-facing message for this bounded state is now explicit: `Connection state: <state>. Wait for the session UI to establish session control before sending input or resizing.`
...
- The product now supports one shipped composer-placement contract for reducing accidental sends to the wrong terminal by making the composer block track terminal context more explicitly.
- Two global composer-placement modes are now live:
...
- A per-terminal pin contract is now live only for `active-overlay`:
...
- `CMP-402` through `CMP-404` are now also complete:
...
- backend coverage: `97.37%` line / `90.82%` to `90.85%` branch / `87.51%` funcs across two back-to-back full coverage runs
- frontend coverage: `97.42%` line / `90.58%` branch / `81.77%` funcs
...
- `npm --prefix frontend run test:coverage` is green on the current tree and exits cleanly with frontend totals `97.42%` line / `90.58%` branch / `81.77%` funcs.
- `npm --prefix backend run test:coverage` passed in two fresh back-to-back runs on 2026-05-16:
  - the prior hard failure at `backend/test/ws.integration.test.js` `WS snapshot and session events preserve trace and correlation continuity` did not reproduce
  - however, the two passing runs still drifted slightly in branch totals (`90.85%` then `90.82%`)
  - the drift landed in backend runtime/session-control lifecycle rows including `backend/src/runtime-session-control-authority.js`, `backend/src/runtime-session-event-authority.js`, `backend/src/runtime.js`, `backend/src/session-manager-lifecycle.js`, `backend/src/session-manager-pty-runtime.js`, and `backend/src/session-manager-startup-runtime.js`
  - current conclusion: the backend coverage lane is no longer hard-red, but its repeated branch evidence is still not fully deterministic
...
- backend:
  - `backend/src/runtime.js`: `2135` lines, `87.21%` line / `80.51%` branch
...
- frontend:
  ...
  - `frontend/src/public/layout-profile-runtime-controller.js`: `433` lines, `93.53%` line / `89.78%` branch / `67.27%` funcs
  - `frontend/src/public/layout-runtime-state.js`: `260` lines, `98.08%` line / `92.80%` branch

Current active tasks:

- `QLT-365` Owner `BE`: stabilize the remaining repeated backend coverage drift across `backend/src/runtime.js`, `backend/src/runtime-session-control-authority.js`, `backend/src/runtime-session-event-authority.js`, and adjacent WebSocket/session-control runtime seams so back-to-back `npm --prefix backend run test:coverage` runs produce stable branch evidence instead of shifting file totals between passes.
- `QLT-372` Owner `BE`: harden retained exited-session lifecycle and restore coverage across `backend/src/session-manager-launch-runtime.js`, `backend/src/session-manager-lifecycle.js`, `backend/src/session-manager-session-runtime.js`, and `backend/src/runtime-startup-restore.js`, including unexpected PTY exit persistence, replay/metric shaping, and explicit restartability after restore.
...
- `QLT-371` Owner `QA`: revalidate and document repeated backend/frontend/root full-lane evidence after `QLT-365`, `QLT-372`, and `QLT-366` through `QLT-370`, including back-to-back `npm --prefix backend run test:coverage` runs to prove the remaining backend coverage drift is gone.

## H185 Session Lifecycle Stabilization Closeout

- `v0.4.0-H185` is now fully closed.
- `STAB-502` closed by the combined reconnect/control hardening now on `main`:
  - the frontend session-control layer fails closed until the transport state is truly `connected`
  - the composer-placement bootstrap tolerates the pre-attach operator-client race instead of poisoning later initialization
  - the trusted-local WebSocket path keeps a stable client identity across frontend-only refreshes
  - the command-composer/runtime-state layer now clears stale unrelated errors when a current send or blocked-send action supersedes them
- `STAB-503` closed by the lifecycle hardening now on `main`:
  - create-session now carries the active `deckId` in the initial backend request instead of relying on a race-prone immediate move
  - stale or ghost cards can be deleted locally when the live backend session record is already gone
  - exited cards relaunch through the header `Play` action via `restartSession(...)`
  - persisted stopped sessions now really stay stopped across restore because `initialState` is forwarded through the restore-create facade
- `STAB-504` closed:
  - `npm --prefix frontend run test` now exits cleanly with `1052` passing tests
  - `npm --prefix frontend run test:coverage` now exits cleanly and feeds the root coverage gate successfully
  - `npm run test` and `npm run test:coverage:check` are trustworthy again on the current tree
- `STAB-505` closed by focused acceptance and integration coverage now locked into the tree:
  - `frontend/test/app.test.js` now explicitly covers the fail-closed pre-attach send phase, the later connected send phase, and the stabilized custom-command transport sequence
  - `frontend/test/app-runtime-composition-controller.test.js` now models connected reclaim/retry behavior explicitly through the composition helper seam
  - backend/runtime integration coverage continues to lock down stopped-state restore persistence and related lifecycle restore semantics through `backend/test/runtime.integration.test.js`, `backend/test/runtime-startup-restore.test.js`, and `backend/test/session-manager.test.js`
...
- `QLT-355` extracted shared layout/workspace apply plus visible-session orchestration into `frontend/src/public/layout-workspace-orchestration-state.js`, so `frontend/src/public/layout-profile-runtime-controller.js`, `frontend/src/public/layout-workspace-runtime-state.js`, and `frontend/src/public/split-layout-runtime-controller.js` no longer carry that logic in partially duplicated form.
- `QLT-356` tightened retained runtime/operator coverage across `frontend/test/connection-profile-draft-state.test.js`, `frontend/test/layout-split-layout-runtime-state.test.js`, and `frontend/test/session-terminal-runtime-controller.test.js`.
- The frontend closeout also surfaced and fixed one real extraction regression: `frontend/src/public/app-runtime-startup-helper-assembly.js` initially rebuilt helper closures but failed to forward several bootstrap collaborators into `createAppRuntimeStartupComposition(...)`. The validated fix now forwards the underlying `traceDebugController`, `sessionControlRuntimeController`, `trustedLocalClientRuntimeController`, `startupBackupRuntimeController`, and `appCommandUiFacadeController` so trusted-local bootstrap, startup backup handling, and app-level error rendering continue to work through the extracted seam.
- `frontend/test/app.test.js` now waits for the actual post-loading render contract before asserting create/delete error visibility and live activity state. That is intentional: `frontend/src/public/ui/workspace-render-controller.js` prioritizes `loading` over `error`, so app integration tests must not treat the first completed `/sessions` response as proof that error text is already eligible to render.

Not promoted from the same review:

- `scripts/analyze-pty-write-eintr.mjs`
- `scripts/analyze-startup-timeline.mjs`
- root live tooling follow-up in `scripts/lib/coverage-report.mjs` and `scripts/scaffold-ui-module.mjs`
- transport-only messaging follow-up work from `TODO-OUTLOOK.md`

They remain intentionally deferred because the diagnostics are not live product/runtime authority paths, the root live-tooling lane is already above the near-term pressure threshold, and the messaging third-attempt chain still depends on separate deferred product decisions.
...
The latest whole-repo validation on 2026-05-06 reran `npm --prefix frontend run test:coverage`, `npm run docs:check`, `npm run lint`, `npm run test`, `npm run test:coverage:check`, and `git diff --check` on top of the `QLT-304` extraction tree. The validated coverage totals are root tooling `92.82%` line / `77.09%` branch, backend `96.25%` line / `89.39%` branch, and frontend `97.39%` line / `90.02%` branch.

## Repo-Wide Quality Review Follow-Up (`v0.4.0-H173`)

On 2026-05-06, a fresh whole-repo quality and coverage review was rerun after the multiline shell-send transport fix. `v0.4.0-H173` is now fully completed on top of that review, closing `QLT-302` through `QLT-307`.

Validated closeout totals on the `H173` tree:

- root tooling: `92.82%` line / `77.09%` branch
- backend: `96.26%` line / `89.35%` branch
- frontend: `97.45%` line / `90.27%` branch

Most relevant retained hotspots after the closeout:

- Backend:
  - `backend/src/runtime.js`: `3280` lines, `85.76%` line / `77.79%` branch coverage after the `QLT-302` normalization extraction
  - `backend/src/runtime-library-normalization.js`: `1724` lines, `79.35%` line / `74.32%` branch coverage
- Frontend:
  - `frontend/src/public/app-runtime-composition-controller.js`: `1692` lines and still the highest-risk retained FE composition monolith after the `QLT-304` initialization/access extraction
  - `frontend/src/public/connection-profile-runtime-controller.js`: `355` lines after the `QLT-305` presentation extraction, with focused seam coverage validating the reduced controller at `100.00%` line / `52.69%` branch while the new presentation seam carries the remaining branch-heavy state shaping
  - `frontend/src/public/workspace-preset-runtime-controller.js`: `249` lines, `89.16%` line / `93.22%` branch coverage after the `QLT-306` extraction
  - `frontend/src/public/workspace-preset-runtime-state.js`: `980` lines, `94.80%` line / `82.10%` branch coverage
  - `frontend/src/public/command-palette-runtime-controller.js`: `613` lines, `96.90%` line / `81.61%` branch coverage
  - `frontend/src/public/slash-workflow-runtime-controller.js`: `412` lines, `91.75%` line / `80.77%` branch coverage
  - `frontend/src/public/ui/session-terminal-runtime-controller.js`: `824` lines, `92.35%` line / `85.23%` branch coverage
...
No follow-up wave is promoted yet after the `H173` closeout. `TODO.md` and `ROADMAP.md` are back to no active tasks and no active or queued wave.
