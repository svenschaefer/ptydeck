...
- Security/auth hardening from the docs review is now partially promoted into the active near-term block `v0.3.0-H15` (`ENT-026` ... `ENT-028`), while broader auth/tenant themes remain deferred in `TODO-OUTLOOK.md`.
...
- Active next cycles: `v0.3.0-H15`.
- H15 implementation status: WebSocket query-token transport has been replaced in the FE/BE code path by one-time WS tickets carried in `Sec-WebSocket-Protocol`, and config now uses explicit `AUTH_MODE`; milestone closure is still pending backend coverage validation.
