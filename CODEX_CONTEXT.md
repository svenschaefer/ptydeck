...
Last updated: 2026-03-27 (`v0.4.0-H28` remains the active delivery wave for the split-layout foundation, with `UX-011` now completed and `UX-012` plus `UX-013` still open; `UX-011` extended the persisted layout-profile and workspace-preset contract so each deck can store normalized split-layout trees plus ordered session assignments in `deckSplitLayouts`, with deterministic invalid-node fallback plus deleted-session/deleted-deck cleanup, and the frontend profile/preset preservation runtimes now keep that contract intact while the actual split-pane renderer remains deferred to `UX-012`; `v0.4.0-H27` is fully completed with `UX-008`, `UX-009`, and `UX-010` closed for workspace-group broadcast input; the frontend now exposes a dedicated broadcast-input runtime in `frontend/src/public/broadcast-input-runtime-controller.js`, wired through the app runtime/bootstrap composition plus the command-target and command-composer runtimes, so ordinary composer sends can target the active or explicitly selected workspace group on the active deck through `/broadcast status`, `/broadcast off`, and `/broadcast group [group]`; broadcast mode deliberately reuses the persisted workspace-preset/session-group contract from `v0.4.0-H26` instead of inventing a second grouping model, the one-line composer target summary now reflects active group broadcast, and explicit `@target ...` direct routing still bypasses broadcast mode deterministically; `v0.4.0-H26` remains fully completed with `UX-005`, `UX-006`, and `UX-007` closed for session grouping and workspace presets, including the persisted backend contract for `activeDeckId`, optional linked `layoutProfileId`, explicit per-deck ordered session-group definitions, active-group markers, deleted-reference cleanup, and restart persistence plus the frontend runtime that applies that contract through `frontend/src/public/workspace-preset-runtime-controller.js`, sidebar preset/group actions, deck-sidebar and session-grid group resolution, and slash commands (`/workspace list`, `/workspace save <name>`, `/workspace apply <preset>`, `/workspace rename <preset> <name>`, `/workspace delete <preset>`); slash help and overview output include the direct-route marker `@` alongside `>` and `/`, so the visible command surface is represented as `Commands: @ > / ...`; `v0.4.0-H25` remains fully completed with `QLT-144`, `QLT-145`, `QLT-146`, `QLT-147`, `QLT-148`, and `QLT-149` closed; direct terminal typing stability hotfixes from the recent post-H24 incident remain in place, including the final deck-independent reorder guard in `frontend/src/public/ui/session-grid-controller.js`; persisted frontend stream debug tracing remains available in debug mode via `window.__PTYDECK_STREAM_DEBUG__`; full local quality gate green)
Owner: `CODY`
Documentation sync status: all repository markdown files reviewed and aligned on 2026-03-27, including the `UX-011` closeout sync inside the still-active `v0.4.0-H28` wave, the earlier `v0.4.0-H27` closeout sync for `UX-008` through `UX-010`, the `v0.4.0-H28` promotion sync for `UX-011` through `UX-013`, the earlier `v0.4.0-H26` closeout sync for `UX-005` through `UX-007`, the slash-help `@` overview correction, the earlier `v0.4.0-H25` closeout sync for `QLT-144` through `QLT-149`, the recent direct-terminal-typing hotfix sync in `frontend/src/public/ui/session-grid-controller.js`, TODO/ROADMAP separation validation, explicit open-task ownership validation, and persistent context capture for the current active-wave H28 roadmap state.
...
- The active H28 backend split-layout baseline is now persisted in `deckSplitLayouts` on both layout profiles and workspace presets: each deck can store a normalized `row` / `column` / `pane` tree plus ordered `paneSessions`, restore paths preserve persisted assignments long enough to survive restart normalization, and cleanup removes assignments that reference deleted sessions or decks without inventing a second frontend-only split-layout model.
