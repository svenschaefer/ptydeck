...
Last updated: 2026-03-27 (`v0.4.0-H30` remains the active delivery wave for parameterized custom command templates, with `CMD-001` now completed and `CMD-002` plus `CMD-003` still open after the full closeout of `v0.4.0-H29`; the backend custom-command contract now supports both `plain` and explicit `template` variants through the existing persisted API surface, with deterministic placeholder validation, strict built-in variable allowlisting, restart-safe restore, and backward compatibility for legacy plain commands in `backend/src/runtime.js`, `backend/src/validation.js`, and `backend/openapi/openapi.yaml`; `v0.4.0-H29` is fully completed with `UX-014`, `UX-015`, and `UX-016` closed for control-plane and execution-plane separation: the backend layout-profile and workspace-preset contract persists deterministic control-pane workspace chrome fields (`controlPaneVisible`, `controlPanePosition`, `controlPaneSize`) alongside the completed split-layout execution model, while the frontend renders a dedicated operator control pane through `frontend/src/public/control-pane-runtime-controller.js`, wired via the app runtime composition so composer, search, and workspace controls live outside the terminal execution pane; layout profiles and workspace presets restore control-pane visibility, position, and size together with split-layout execution state, narrow viewports automatically fall back from stored `left` / `right` positions to an effective `bottom` pane, and live resize clamps to the persisted backend contract instead of snapping back to defaults; `v0.4.0-H28` remains fully completed with `UX-011`, `UX-012`, and `UX-013` closed for the split-layout foundation: the backend and persistence contract store per-deck normalized split-layout trees plus ordered pane assignments in `deckSplitLayouts`, optional normalized container `weights`, and deleted-session/deleted-deck fallback rules, while the frontend renders row/column split panes with assignment controls, drag-resize handles, and backend-backed layout-profile/workspace-preset apply/save behavior through `frontend/src/public/split-layout-runtime-controller.js`; pane contents continue to follow current quick-ID ordering inside each pane so `/swap` remains visibly authoritative even in split layouts; `v0.4.0-H27` remains fully completed with `UX-008`, `UX-009`, and `UX-010` closed for workspace-group broadcast input; the frontend exposes a dedicated broadcast-input runtime in `frontend/src/public/broadcast-input-runtime-controller.js`, wired through the app runtime/bootstrap composition plus the command-target and command-composer runtimes, so ordinary composer sends can target the active or explicitly selected workspace group on the active deck through `/broadcast status`, `/broadcast off`, and `/broadcast group [group]`; broadcast mode deliberately reuses the persisted workspace-preset/session-group contract from `v0.4.0-H26` instead of inventing a second grouping model, the one-line composer target summary reflects active group broadcast, and explicit `@target ...` direct routing still bypasses broadcast mode deterministically; slash help and overview output include the direct-route marker `@` alongside `>` and `/`, so the visible command surface is represented as `Commands: @ > / ...`; direct terminal typing stability hotfixes from the recent post-H24 incident remain in place, including the final deck-independent reorder guard in `frontend/src/public/ui/session-grid-controller.js`; persisted frontend stream debug tracing remains available in debug mode via `window.__PTYDECK_STREAM_DEBUG__`; full local quality gate green)
Owner: `CODY`
Documentation sync status: all repository markdown files reviewed and aligned on 2026-03-27, including the `CMD-001` closeout sync inside the active `v0.4.0-H30` wave, the earlier `v0.4.0-H29` closeout sync for `UX-014` through `UX-016`, the `v0.4.0-H30` promotion sync for `CMD-001` through `CMD-003`, the earlier `v0.4.0-H28` closeout sync for `UX-011` through `UX-013`, the earlier `v0.4.0-H27` closeout sync for `UX-008` through `UX-010`, the earlier `v0.4.0-H26` closeout sync for `UX-005` through `UX-007`, the slash-help `@` overview correction, the earlier `v0.4.0-H25` closeout sync for `QLT-144` through `QLT-149`, the recent direct-terminal-typing hotfix sync in `frontend/src/public/ui/session-grid-controller.js`, TODO/ROADMAP separation validation, explicit open-task ownership validation, and persistent context capture for the current active-wave H30 roadmap state.
...
- The active H30 command-template wave is intentionally scoped to the existing persisted custom-command surface: `CMD-001` is completed and establishes explicit `plain` vs `template` persisted custom-command variants, deterministic `{{param:name}}` / `{{var:...}}` placeholder validation, and strict built-in variable allowlisting; the remaining `CMD-002` / `CMD-003` frontend and QA work must reuse that contract rather than introducing a second local-only macro system next to the current `/custom` storage and execution flow.
