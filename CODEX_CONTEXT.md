...
Last updated: 2026-03-27 (`v0.4.0-H25` is now the active wave with `QLT-144`, `QLT-145`, `QLT-146`, `QLT-147`, `QLT-148`, and `QLT-149` open; the next frontend simplification block now explicitly covers two user-selectable per-session theme schemes, one used when a terminal is active and one used when it is inactive, via a persisted dual session-theme contract (`activeThemeProfile` / `inactiveThemeProfile`); the same wave also covers frontend runtime switching between those active and inactive terminal color schemes, removal of terminal-header replay `View` / `DL` actions in favor of slash-command access, extension of send-safety preset enforcement to paste-triggered input flows, collapse of the composer metadata strip to one ` · `-separated line without a visible preset label, and split slash-help output for `/help` plus `/help <topic>`; `v0.4.0-H24` remains fully completed with `QLT-142` and `QLT-143` closed; frontend-local quick-ID swaps now drive visible session ordering across the deck sidebar, workspace grid, and selector-driven navigation, and the quick-ID mapping now survives browser reloads in the same browser storage context without becoming backend-persisted; a follow-up focus-stability hotfix in `frontend/src/public/ui/session-grid-controller.js` now reorders existing terminal cards only when the desired quick-ID order diverges from the current DOM order, so direct typing into an already focused terminal no longer loses xterm focus on each normal render while `/swap`-driven reorder behavior remains visible; `v0.4.0-H23` remains fully completed with `UX-002`, `UX-003`, and `UX-004` closed; the backend now exposes a persisted layout-profile contract through `GET/POST /api/v1/layout-profiles` plus `GET/PATCH/DELETE /api/v1/layout-profiles/{profileId}`, with deterministic normalization for active deck, sidebar visibility, session filter text, and per-deck terminal geometry; frontend runtime now owns one backend-backed layout-profile workflow in `frontend/src/public/layout-profile-runtime-controller.js`, wired through `app-runtime-composition-controller.js`, `app-bootstrap-composition-controller.js`, `index.html`, and `styles.css`, and exposed both through sidebar actions and slash commands (`/layout list`, `/layout save <name>`, `/layout apply <profile>`, `/layout rename <profile> <name>`, `/layout delete <profile>`); applying a layout profile updates active deck, sidebar visibility, session filter text, and deck terminal geometry from the persisted contract without introducing a second client-only layout state model; a follow-up hotfix in that same runtime controller now clears real DOM `SELECT` children correctly before rerendering the backend-backed profile list, so layout-profile entries no longer multiply in the browser UI after reload/save/delete flows and deleting a profile no longer appears broken because stale duplicate `<option>` nodes remain mounted; dedicated backend/frontend regression coverage now spans persistence, restart restore, invalid payload handling, frontend apply/save flows, DOM-like selector rerendering, and focus-preserving session-grid reorder behavior; `v0.4.0-H22` remains completed with `UX-001` closed; the frontend exposes a global command palette reachable via `Ctrl/Cmd+K`, implemented in `frontend/src/public/command-palette-runtime-controller.js` and wired through `app-runtime-composition-controller.js`, `index.html`, and `styles.css`; the palette reuses the existing declarative slash-command schema plus current custom-command, session, and deck state, groups results deterministically as commands/sessions/decks, prefills the central composer for command picks, and activates sessions or decks directly for navigation picks; app-level regression coverage now includes palette shortcut integration in `frontend/test/app.test.js`, and focused controller coverage exists in `frontend/test/command-palette-runtime-controller.test.js`; `v0.4.0-H21` remains completed with `DPR-001` and `DPR-002` closed; the repository now has an explicit ADR process in `docs/adr/` with deterministic numbering, a reusable template, a `scripts/new-adr.sh` generator, a `scripts/check-adr-process.sh` lint gate, and an initial accepted ADR (`0001-local-validation-is-the-authoritative-quality-gate.md`); repository-local delivery templates now exist in `templates/backend-endpoint/` and `templates/frontend-ui-module/`, and `scripts/scaffold-ui-module.mjs` can generate a new frontend controller/test pair aligned with the current `create...Controller()` pattern; `v0.4.0-H20` remains completed with `REP-004` and `REP-005` closed on top of the earlier completed replay-export baseline `REP-001`, `REP-002`, and `REP-003`; the backend exposes `GET /api/v1/sessions/{sessionId}/replay-export` as a deterministic retained-replay-tail export contract with explicit scope/format/content-type markers, stable authz/validation semantics, empty-session and unrestored-session behavior, and truncation metadata preserved across persisted restart-replay tails; the frontend now exposes that retained-tail contract through slash commands (`/replay view`, `/replay export`, `/replay copy`), a discoverable per-session `View` reading-mode action, and a `DL` export action, with the replay viewer, download, and copy flows all reporting retained-size and truncation state explicitly; `DRV-001` introduced a dedicated backend shell-adapter abstraction for CWD tracking with an explicit capability matrix: `bash` remains supported through `PROMPT_COMMAND` marker injection, while `zsh`, `fish`, `posix_sh`, and unknown shells now fall back deterministically to retaining the last known CWD without spawn-env mutation; `DRV-002` added explicit replay/scrollback retention controls with separate in-memory and optional persisted restart-replay limits; `DRV-005` added shell/runtime compatibility regression coverage for `bash`, `zsh`, and `fish` across CWD tracking plus replay/snapshot behavior; latest earlier closeout `v0.4.0-H17` removed active frontend stream-interpretation, header-status scanning, artifact-overlay, attention-surface, and browser-notification runtime remnants so the composed frontend stream path is now limited to raw terminal append, debug tracing, and idle-driven activity clear; the terminal-session header meta row now renders only persisted notes and tags; active/inactive detection is now driven solely by `hasMeaningfulStreamActivity()`, so ANSI-only, control-only, whitespace-only, and invisible redraw chunks do not count as activity while visible micro-redraw glyphs still do; frontend quick IDs remain intentionally frontend-local and non-backend-persisted; `/swap <selectorA> <selectorB>` now also reorders visible session surfaces and survives browser reloads in the same browser storage context; session notes and per-session input-safety profiles are backend-persisted; the frontend command composer applies preset-backed per-session send guardrails with explicit `Send once` override flows; backend `/ready` exposes `booting`, `starting_sessions`, and `ready`; WebSocket upgrades enforce the configured browser-origin allowlist; all top-level root scripts under `scripts/` emit a standardized startup log line enforced by root lint; persisted frontend stream debug tracing remains available in debug mode via `window.__PTYDECK_STREAM_DEBUG__`; full local quality gate green)
Owner: `CODY`
Documentation sync status: all repository markdown files reviewed and aligned on 2026-03-27, including hotfix sync for the session-grid focus-stability fix after H24 quick-ID ordering, hotfix sync for the layout-profile selector DOM-clearing bug in `frontend/src/public/layout-profile-runtime-controller.js`, promotion sync for `QLT-144`, `QLT-145`, `QLT-146`, `QLT-147`, `QLT-148`, and `QLT-149` into the active `v0.4.0-H25` session-theme / composer / help simplification wave, closeout sync for `QLT-142` and `QLT-143` as the completed `v0.4.0-H24` frontend quick-ID swap consistency hotfix wave, closeout sync for `UX-002`, `UX-003`, and `UX-004` as the completed `v0.4.0-H23` persistent-layout-profile wave, closeout sync for `UX-001` as the completed `v0.4.0-H22` command-palette wave, closeout sync for `DPR-001` and `DPR-002` as the completed `v0.4.0-H21` developer-productivity wave, closeout sync for `REP-004` and `REP-005` as the completed `v0.4.0-H20` replay-reading-mode wave, full closeout sync for `REP-001`, `REP-002`, and `REP-003` as the completed `v0.4.0-H19` replay-export baseline, completion sync for `DRV-001`, `DRV-002`, and `DRV-005` as the completed `v0.4.0-H18` shell runtime compatibility wave, completion sync for `QLT-140` and `QLT-141` as `v0.4.0-H17`, `docs/` review-note consolidation, TODO/ROADMAP separation validation, explicit open-task ownership sync, completion sync for `QLT-095`/`QLT-096`, `QLT-097`/`QLT-098`, `QLT-099`, `QLT-100`, `QLT-101`/`QLT-104`/`QLT-105`/`QLT-106`/`QLT-107`, hidden-scroll recovery closure for `QLT-108`/`QLT-109`, `QLT-110`/`QLT-111` terminal search completion, completion sync for the declarative autocomplete milestone (`QLT-112` ... `QLT-115`), completion sync for runtime metadata event consistency (`QLT-116` ... `QLT-118`), closure of the runtime store and contract hardening block (`QLT-119` ... `QLT-122`), completion of the explicit lifecycle-state-model block (`LIF-001` ... `LIF-006`), promotion of the stream-interpretation/plugin follow-up into `TODO.md` / `ROADMAP.md` as `v0.3.0-H13`, completion of `ARC-003` stream-adapter foundation, completion of `ARC-004` deterministic plugin-engine registry foundation, completion of `ARC-005` interpretation-action contract plus dispatcher integration, completion of `ARC-006` built-in stream-interpreter baseline, completion of `ARC-007` artifact-oriented interpretation and session-surface rendering, completion of `ARC-008` regression hardening for the full stream-interpretation foundation, promotion and completion sync for the browser-notification follow-up `v0.3.0-H14` (`LIF-007` ... `LIF-009`), completion of the auth hardening follow-up `v0.3.0-H15` (`ENT-026` ... `ENT-028`) including one-time WS handshake tickets and explicit `AUTH_MODE`, send-body newline normalization hardening so only the configured final terminator emits control-submit sequences, explicit decomposition of deferred plugin/stream-interpretation architecture into implementable `ARC-003` ... `ARC-008` subtasks, the `TODO-OUTLOOK.md` filename correction, docs-derived deferred stack-evolution capture in `TODO-OUTLOOK.md` (`ALT-001` ... `ALT-006`) alongside deferred `REM-001` ... `REM-009`, promotion of the next active block from `TODO-OUTLOOK.md` into `TODO.md` / `ROADMAP.md` as `v0.4.0-H1` (`OBS-001` ... `OBS-004`), completion sync for `v0.4.0-H1` (`OBS-001` ... `OBS-004`) across code/tests/docs, promotion of remaining layered-frontend-architecture tasks from `docs/Codebase Review - Decomposing app js into a Layered Frontend Architecture.md` into active backlog as `v0.4.0-H2` (`ARC-009` ... `ARC-012`), explicit split of `ARC-010` into closeout subgoals `ARC-010A` ... `ARC-010D`, completion sync for the recent `ARC-010` controller-extraction increments (`session-terminal-runtime`, `session-terminal-resize`, `runtime-event`, `session-grid`, `ws-runtime`, `command-composer-runtime`, `auth-bootstrap-runtime`, `terminal-search-runtime`, `deck-runtime`, `session-runtime`, `layout-runtime`, `app-lifecycle`, `session-settings-state`, `command-target-runtime`, `app-runtime-state`, `session-ui-facade`, `app-layout-deck-facade`, `app-session-runtime-facade`, `app-command-ui-facade`, `app-bootstrap-composition-controller`, `app-runtime-composition-controller`, `app-architecture-closeout`), session-artifact overlay UX synchronization (artifacts no longer consume terminal height and can be dismissed until new artifact content arrives), follow-up overlay hardening so the dismiss control stays hidden unless the artifact overlay itself is visible, activity-status priority hardening so richer timer/progress patterns outrank lower-detail matches, split-chunk redraw reconstruction for activity-status parsing so status upgrades remain stable across partial terminal updates, roadmap versioning clarification for active `v0.4.x` waves, full local quality-gate confirmation for the latest ARC-010 increment, status clarification that `H2` is now the active wave, `ARC-010A` closeout sync across planning docs, `ARC-010B` closeout sync across planning docs, `ARC-010C` closeout sync across planning docs, `ARC-010D` closeout sync across planning docs, promotion of the declarative command-contract block as `v0.4.0-H4`, closeout sync for `DRV-003A`, `DRV-003B`, `DRV-003C`, closeout sync for `DRV-004`, stream-status retention hardening so richer timer/progress detections cannot be downgraded by later weaker redraw chunks inside the same activity window, session-status ticker formatting hardening so live durations remain human-readable, persisted frontend stream-debug trace support in debug mode, promotion of `QLT-125` into the new active wave `v0.4.0-H5` for empty/no-op stream-activity filtering, completion sync for `QLT-125` with `hasMeaningfulStreamActivity()` gating frontend activity bumps for ANSI-only/control-only/whitespace-only chunks, promotion and completion sync for `QLT-126` and `QLT-127` as the startup-warmup / bootstrap-deferral `v0.4.0-H6` block, promotion and completion sync for `ENT-017` as the `v0.4.0-H7` WebSocket origin-allowlist hardening block, completion sync for `QLT-128` as the `v0.4.0-H8` script-execution traceability block, completion sync for `QLT-129` as the `v0.4.0-H9` invisible-stream-activity filtering hardening block, completion sync for `QLT-133` as the `v0.4.0-H13` attention-header silence hotfix, completion sync for `QLT-134` as the `v0.4.0-H14` frontend-local quick-ID swap command, promotion of `QLT-135` and `QLT-136` into the active `v0.4.0-H15` persistent session-note wave, completion sync for `QLT-135` and `QLT-136` as the completed `v0.4.0-H15` persistent session-note wave, promotion of `QLT-137`, `QLT-138`, and `QLT-139` into the queued `v0.4.0-H16` target-clarity and send-safety wave, H16 scope refinement so the open block now explicitly requires persisted safety-profile fields, preset mappings, parser-backed shell-syntax gating for opted-in shell sessions, incomplete-shell confirmations, natural-language shell confirmations, dangerous-command confirmations, multiline or oversized input confirmations, and recent-target-switch confirmations, completion sync for `QLT-137` as the active-border-only hotfix, completion sync for `QLT-138` as the backend-persisted per-session input-safety profile contract with OpenAPI/type alignment and restart/restore preservation, and a final governance audit confirming no TODO/ROADMAP drift after the latest cycle.
...
- `TODO.md` currently carries the active `v0.4.0-H25` implementation tasks (`QLT-144` ... `QLT-149`); completed history remains exclusively in `DONE.md`.
...
- Frontend replay export is now an explicit operator workflow rather than an undocumented API helper: terminal cards expose a `DL` toolbar action, slash commands support `/replay export [selector|active]` and `/replay copy [selector|active]`, and command feedback reports retained-size plus truncation state explicitly.
- Frontend replay reading mode now exists on top of the same retained-tail contract: terminal cards expose a `View` action, slash commands support `/replay view [selector|active]`, and the viewer dialog can refresh, copy, and download the currently retained replay tail without implying full-history or full-process-state access.
...
- The next active UI simplification wave intentionally removes terminal-header replay buttons in favor of slash-command replay access, applies guarded-send behavior to paste-triggered input as well as explicit send, collapses composer metadata to one line, and introduces topic-specific slash-help output.
