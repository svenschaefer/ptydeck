...
Last updated: 2026-03-27 (`v0.4.0-H32` is now fully completed with `CMD-007`, `CMD-008`, and `CMD-009` closed for scoped custom-command sets: the backend now persists `global` / `project` / `session` command scopes with explicit precedence metadata and session binding semantics, while the frontend now supports scoped `/custom` save/show/preview/remove flows, scoped autocomplete references, scope-aware command-palette visibility, and per-target precedence resolution `session > project > global`; `v0.4.0-H33` is now the active delivery wave for command namespaces and scriptability with `CMD-010`, `CMD-011`, and `CMD-012`; `v0.4.0-H31` remains fully completed with `CMD-004`, `CMD-005`, and `CMD-006` closed for fuzzy and personalized command suggestions; `v0.4.0-H30` remains fully completed with `CMD-001`, `CMD-002`, and `CMD-003` closed for parameterized custom command templates; `v0.4.0-H29` remains fully completed with `UX-014`, `UX-015`, and `UX-016` closed for control-plane and execution-plane separation; `v0.4.0-H28` remains fully completed with `UX-011`, `UX-012`, and `UX-013` closed for the split-layout foundation; `v0.4.0-H27` remains fully completed with `UX-008`, `UX-009`, and `UX-010` closed for workspace-group broadcast input; slash help and overview output include the direct-route marker `@` alongside `>` and `/`, so the visible command surface is represented as `Commands: @ > / ...`; direct terminal typing stability hotfixes from the recent post-H24 incident remain in place, including the final deck-independent reorder guard in `frontend/src/public/ui/session-grid-controller.js`; persisted frontend stream debug tracing remains available in debug mode via `window.__PTYDECK_STREAM_DEBUG__`; full local quality gate green)
Owner: `CODY`
Documentation sync status: all repository markdown files reviewed and aligned on 2026-03-27, including the full `v0.4.0-H32` closeout sync for `CMD-007` through `CMD-009`, the `v0.4.0-H33` promotion sync for `CMD-010` through `CMD-012`, the earlier `v0.4.0-H31` closeout sync for `CMD-004` through `CMD-006`, the earlier `v0.4.0-H30` closeout sync for `CMD-001` through `CMD-003`, the earlier `v0.4.0-H29` closeout sync for `UX-014` through `UX-016`, the earlier `v0.4.0-H28` closeout sync for `UX-011` through `UX-013`, the earlier `v0.4.0-H27` closeout sync for `UX-008` through `UX-010`, the earlier `v0.4.0-H26` closeout sync for `UX-005` through `UX-007`, the slash-help `@` overview correction, the earlier `v0.4.0-H25` closeout sync for `QLT-144` through `QLT-149`, the recent direct-terminal-typing hotfix sync in `frontend/src/public/ui/session-grid-controller.js`, TODO/ROADMAP separation validation, explicit open-task ownership validation, and persistent context capture for the current active-wave H33 roadmap state.
...
- The active H33 command-plane wave should keep one registry/executor path authoritative: namespaced command aliases (for example `/session.new`, `/deck.switch`, `/custom.show`) must be layered onto the existing slash-command schema and executor rather than creating a second command-dispatch system, and multi-command script execution must remain frontend command-plane orchestration only, not PTY-stream scripting.
