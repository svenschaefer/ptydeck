...
- The active H30 command-template wave is intentionally scoped to the existing persisted custom-command surface: custom commands will gain an explicit template-capable variant plus strict opt-in built-in variable expansion, rather than introducing a second local-only macro system next to the current `/custom` storage and execution flow.
