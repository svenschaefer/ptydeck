...
Last updated: 2026-03-27 (`v0.4.0-H31` is now the active delivery wave for fuzzy and personalized command suggestions after the full closeout of `v0.4.0-H30`; `v0.4.0-H30` is fully completed with `CMD-001`, `CMD-002`, and `CMD-003` closed for parameterized custom command templates: the backend custom-command contract supports both `plain` and explicit `template` variants through the existing persisted API surface, while the frontend now supports explicit template definitions, `/custom preview`, template-aware `/custom show`, composer preview rendering, deterministic `key=value` parameter parsing plus optional `-- <targetSelector>` routing, and per-target template execution using only the backend-allowlisted built-in variables; plain custom commands remain backward-compatible and continue to execute without a second local-only macro model; the next active H31 wave is intentionally frontend-local and deterministic: fuzzy command suggestions must preserve exact-prefix priority and stable fallback ordering first, then layer browser-local recency personalization on top without inventing inconsistent ranking paths between composer autocomplete and the command palette; `v0.4.0-H29` remains fully completed with `UX-014`, `UX-015`, and `UX-016` closed for control-plane and execution-plane separation; `v0.4.0-H28` remains fully completed with `UX-011`, `UX-012`, and `UX-013` closed for the split-layout foundation; `v0.4.0-H27` remains fully completed with `UX-008`, `UX-009`, and `UX-010` closed for workspace-group broadcast input; slash help and overview output include the direct-route marker `@` alongside `>` and `/`, so the visible command surface is represented as `Commands: @ > / ...`; direct terminal typing stability hotfixes from the recent post-H24 incident remain in place, including the final deck-independent reorder guard in `frontend/src/public/ui/session-grid-controller.js`; persisted frontend stream debug tracing remains available in debug mode via `window.__PTYDECK_STREAM_DEBUG__`; full local quality gate green)
Owner: `CODY`
Documentation sync status: all repository markdown files reviewed and aligned on 2026-03-27, including the full `v0.4.0-H30` closeout sync for `CMD-001` through `CMD-003`, the `v0.4.0-H31` promotion sync for `CMD-004` through `CMD-006`, the earlier `v0.4.0-H29` closeout sync for `UX-014` through `UX-016`, the earlier `v0.4.0-H28` closeout sync for `UX-011` through `UX-013`, the earlier `v0.4.0-H27` closeout sync for `UX-008` through `UX-010`, the earlier `v0.4.0-H26` closeout sync for `UX-005` through `UX-007`, the slash-help `@` overview correction, the earlier `v0.4.0-H25` closeout sync for `QLT-144` through `QLT-149`, the recent direct-terminal-typing hotfix sync in `frontend/src/public/ui/session-grid-controller.js`, TODO/ROADMAP separation validation, explicit open-task ownership validation, and persistent context capture for the current active-wave H31 roadmap state.
...
- The active H31 suggestion-ranking wave must remain deterministic first: exact-prefix matches stay authoritative, fuzzy ranking broadens recall only after that exact-prefix layer, and browser-local recency personalization can reorder only otherwise-comparable results while preserving stable fallback ordering across composer autocomplete and command-palette surfaces.
