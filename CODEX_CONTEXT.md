...
Last updated: 2026-05-07 (`v0.4.0-H178` is now active in `TODO.md` / `ROADMAP.md` after a fresh repo-wide quality review promoted the next retained backend and frontend pressure points. Current evidence still shows the strongest remaining implementation pressure in `backend/src/runtime.js`, `backend/src/runtime-library-normalization.js`, `backend/src/session-manager.js`, `backend/src/runtime-session-resource-authority.js`, `frontend/src/public/app-runtime-composition-controller.js`, `frontend/src/public/connection-profile-draft-state.js`, `frontend/src/public/split-layout-runtime-controller.js`, `frontend/src/public/layout-profile-runtime-controller.js`, `frontend/src/public/layout-runtime-state.js`, `frontend/src/public/layout-workspace-runtime-state.js`, and `frontend/src/public/ui/session-terminal-runtime-controller.js`. The backend runtime now delegates retained replay export/excerpt shaping, session file-transfer guard rails, and restore-create forwarding into `backend/src/runtime-session-resource-authority.js` in addition to custom-command catalog, deck catalog, session-control fallback-owner/state shaping, and persisted runtime snapshot serialization in `backend/src/runtime-catalog-authority.js`, startup-readiness state in `backend/src/runtime-startup-readiness.js`, session-control client/session authority in `backend/src/runtime-session-control-authority.js`, share/spectator request-access and metrics helpers in `backend/src/runtime-access-policy.js` and `backend/src/runtime-metrics.js`, deck/session-state shaping in `backend/src/runtime-session-state.js`, persisted startup/restore authority in `backend/src/runtime-startup-restore.js`, share/layout/connection/workspace authority in `backend/src/runtime-library-authority.js`, retained catalog/state normalization in `backend/src/runtime-library-normalization.js`, and SSH trust lifecycle handling in `backend/src/runtime-ssh-trust.js`; the backend session manager now delegates trace/activity/input/resize/signal orchestration into `backend/src/session-manager-terminal-runtime.js` in addition to create/restart/close/guardrail orchestration in `backend/src/session-manager-session-runtime.js`, cleanup in `backend/src/session-manager-cleanup.js`, SSH remote-runtime state shaping in `backend/src/session-manager-remote-runtime.js`, startup/post-start input lifecycle in `backend/src/session-manager-startup-runtime.js`, create/update/restart lifecycle state shaping in `backend/src/session-manager-lifecycle.js`, replay-output append/export/excerpt handling in `backend/src/session-manager-replay-runtime.js`, app-identity refresh and output-heuristic arbitration in `backend/src/session-manager-app-identity-runtime.js`, and SSH launch/reconnect orchestration in `backend/src/session-manager-launch-runtime.js`; direct local PowerShell sessions are now first-class local launch aliases, with `backend/src/session-launch-spec.js` normalizing `/new powershell` to `powershell.exe` and `/new pwsh` to `pwsh.exe` while `backend/src/shell-adapter.js` classifies those launchers explicitly as the `powershell` family with unsupported cwd/shell-block tracking and the existing retain-last-known-cwd fallback; trusted-local handoff still resolves runtime client id through the session-control authority seam and fails closed on stale session takeover targets; SSH launches still pin `HostKeyAlgorithms` to the trusted host-key types for the selected target, pass an absolute managed `UserKnownHostsFile` path into spawned `ssh` processes, preserve canonical padded host-key base64 so the rendered managed `ssh_known_hosts` file stays parseable by OpenSSH, keep secret-backed auth on one masked action-dialog seam, expose slash-command host-key lifecycle management through `/ssh hostkey ...`, surface first-connect and rotation trust guidance directly in `Connections`, and extend one-shot `/ssh ...` parity with `--deck`, `--cwd`, and `--command`; multiline terminal sends still distinguish shell-family sessions from coding-agent/unknown sessions so the frontend can normalize internal line separators to the configured terminator for shell-targeted `Send`, custom-command, and quick-send flows instead of always forcing raw `LF` inside multiline payloads; the frontend runtime composition still delegates initialization/error/reclaim state plus session-access composition into `frontend/src/public/app-runtime-initialization-access-composition.js`, deck/session-grid action orchestration into `frontend/src/public/app-runtime-session-grid-actions.js`, retained session/grid/operator surface wiring into `frontend/src/public/app-runtime-session-surface-assembly.js`, retained test-hook installation and collaborator override wiring into `frontend/src/public/app-runtime-composition-helper-assembly.js`, control-pane/layout-profile/connection-profile/workspace-preset controller assembly into `frontend/src/public/app-runtime-operator-controller-assembly.js`, and the existing trusted-local handoff/layout, bootstrap wiring, runtime-event recovery, operator-support assembly, and startup/workflow/palette/initialization seams out of `frontend/src/public/app-runtime-composition-controller.js`; `frontend/src/public/command-executor.js` still delegates retained runtime/router helpers into `frontend/src/public/command-executor-runtime-router.js`; `frontend/src/public/connection-profile-runtime-controller.js` still delegates saved-profile selection, draft/trust presentation state, guarded operator fallbacks, and retained presentation/lifecycle/action assembly into `frontend/src/public/connection-profile-runtime-presentation.js` and `frontend/src/public/connection-profile-runtime-assembly.js`; `frontend/src/public/workspace-preset-runtime-controller.js` still delegates workspace snapshot/group/layout orchestration into `frontend/src/public/workspace-preset-runtime-state.js`, and that workspace/layout snapshot capture now flows through `frontend/src/public/layout-workspace-runtime-state.js`; retained split-layout/layout-profile state shaping still flows through `frontend/src/public/layout-runtime-state.js` instead of duplicated helper logic in both layout controllers.)
...
## Repo-Wide Quality Review - 2026-05-07

Fresh repo-wide evidence on the current tree after the `H177` closeout and the `H178` promotion pass:

- root tooling coverage: `92.84%` line / `77.05%` branch / `94.83%` funcs
- backend coverage: `97.00%` line / `90.54%` branch / `88.64%` funcs
- frontend coverage: `97.63%` line / `90.72%` branch / `82.27%` funcs
...
- backend:
  - `backend/src/runtime.js`: `2441` lines, `84.64%` line / `77.80%` branch, still the largest retained backend authority surface
  - `backend/src/runtime-library-normalization.js`: `1724` lines, `88.28%` line / `84.46%` branch
  - `backend/src/session-manager.js`: `841` lines, `95.72%` line / `79.09%` branch
  - `backend/src/runtime-session-resource-authority.js`: `372` lines, `86.02%` line / `72.41%` branch
  - `backend/src/session-manager-terminal-runtime.js`: `287` lines, `100.00%` line / `90.65%` branch
  - `backend/src/runtime-session-control-authority.js`: `445` lines, `99.55%` line / `93.29%` branch
  - `backend/src/runtime-library-authority.js`: `528` lines, `96.95%` line / `87.88%` branch
  - `backend/src/runtime-session-state.js`: `275` lines, `97.81%` line / `86.84%` branch
- frontend:
  - `frontend/src/public/app-runtime-composition-controller.js`: `1333` lines, `98.05%` line / `82.05%` branch, still the largest retained frontend runtime-composition monolith
  - `frontend/src/public/connection-profile-draft-state.js`: `680` lines, `97.35%` line / `81.34%` branch
  - `frontend/src/public/split-layout-runtime-controller.js`: `657` lines, `95.43%` line / `77.93%` branch
  - `frontend/src/public/layout-profile-runtime-controller.js`: `504` lines, `93.06%` line / `85.28%` branch
  - `frontend/src/public/ui/session-terminal-runtime-controller.js`: `824` lines, `92.96%` line / `86.33%` branch
  - `frontend/src/public/layout-runtime-state.js`: `260` lines, `91.92%` line / `81.72%` branch
  - `frontend/src/public/layout-workspace-runtime-state.js`: `292` lines, `96.58%` line / `79.69%` branch
  - `frontend/src/public/ws-runtime-controller.js`: `100.00%` line / `93.33%` branch, no longer a first-order pressure point

Current active tasks after the `H178` promotion:
