...
- The next planned control-pane UX increment is a per-session send-history browser opened from the meta strip next to `Manage` / position / hide. The intended baseline is frontend-local and bounded rather than backend-persisted: store full payloads only in bounded local history keyed by session, render only truncated previews in the default list, require explicit expansion for long entries, and allow restoring a selected entry back into the command input box without auto-sending it.
