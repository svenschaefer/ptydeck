...
- The active H34 slash-workflow wave should keep the workflow layer deterministic and frontend-only at first: `SWF-001` through `SWF-003` should build a strict workflow AST, sequential execution state machine, and abortable wait primitives on top of the existing slash-command registry/executor instead of inventing a second parser or hidden PTY-stream automation layer.
