...
- Remote sessions now reuse the same backend session contract as local sessions: `Session.kind` is `local|ssh`, `Session.remoteConnection` persists only normalized non-secret SSH metadata (`host`, `port`, optional `username`), and restore/restart logic must stay contract-compatible across both session kinds.
...
- The active H38 remote-session scope is intentionally backend-first: remote `ssh` session-kind support, auth handling, and host-key trust should land on one persisted session contract before later FE-only connection-profile, sharing, or transfer UX is layered on top.
