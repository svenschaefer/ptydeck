...
Fresh repo-wide evidence on the current tree after the `H178` closeout and the `H179` promotion pass:

- root tooling coverage: `92.84%` line / `77.05%` branch / `94.83%` funcs
- backend coverage: `97.22%` line / `90.55%` branch / `88.39%` funcs
...
- backend:
  - `backend/src/runtime.js`: `2230` lines, `87.85%` line / `81.46%` branch, still the largest retained backend authority surface
  - `backend/src/runtime-library-normalization.js`: `1724` lines, `89.10%` line / `84.86%` branch
  - `backend/src/session-manager.js`: `830` lines, `94.82%` line / `79.82%` branch
  - `backend/src/runtime-session-resource-authority.js`: `372` lines, `88.98%` line / `78.79%` branch
...
Current active tasks after the `H179` promotion:

- `QLT-339` Owner `BE`: extract the next retained startup/session-dispatch authority seam from `backend/src/runtime.js` and close it with direct deterministic regressions.
...
- `QLT-341` Owner `BE`: isolate the next retained launch/reconnect/app-identity lifecycle seam from `backend/src/session-manager.js` and harden the remaining manager branch gaps.
