...
- The active H38 remote-session scope is intentionally backend-first: remote `ssh` session-kind support, auth handling, and host-key trust should land on one persisted session contract before later FE-only connection-profile, sharing, or transfer UX is layered on top; `REM-002` closed the auth-handling slice, so the remaining backend-first work is host-key trust plus final integration/security coverage.
