...
- `TODO.md`: active promoted tasks are now `QLT-248` and `QLT-249`, covering the remaining frontend operator command/workflow surfaces plus the retained session-UI glue that still remain product-relevant after the two backend seam cuts, the frontend runtime-state seam extraction, and the operator layout/profile/settings hardening in `v0.4.0-H159`.
...
- frontend: `96.39%` line / `88.54%` branch
...
The latest closeout validation on 2026-04-29 for `QLT-247` passed `node --test frontend/test/deck-runtime-controller.test.js frontend/test/file-transfer-runtime-controller.test.js frontend/test/layout-profile-runtime-controller.test.js frontend/test/session-settings-state-controller.test.js frontend/test/split-layout-runtime-controller.test.js frontend/test/connection-profile-runtime-controller.test.js`, `npm --prefix frontend run test:coverage`, `npm run docs:check`, `npm run lint`, `npm run test`, `npm run test:coverage:check`, and `git diff --check`. The validated coverage totals are root tooling `92.77%` line / `76.81%` branch, backend `94.65%` line / `87.84%` branch, and frontend `96.39%` line / `88.54%` branch.
