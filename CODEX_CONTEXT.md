...
- `TODO.md`: no active promoted tasks remain.
...
- `TODO.md`: the only active promoted task is `QLT-289`.
- `ROADMAP.md`: active wave is `v0.4.0-H170`; no queued next wave is currently promoted.

## Repo-Wide Quality Review Follow-Up (`v0.4.0-H170` active)

On 2026-05-05, the next quality-follow-up wave was promoted directly from a fresh rerun on the validated `H169` closeout tree rather than from stale pre-closeout hotspot snapshots.

Validated top-line coverage on the currently active tree:

- root tooling: `92.82%` line / `77.09%` branch
- backend: `95.56%` line / `89.35%` branch
- frontend: `97.30%` line / `89.65%` branch
...
Completed inside the active wave so far:
