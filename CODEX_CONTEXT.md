...
- backend coverage: `96.89%` line / `90.27%` branch / `88.92%` funcs
...
- backend:
  - `backend/src/runtime.js`: `2788` lines, `84.45%` line / `76.93%` branch
  - `backend/src/runtime-library-normalization.js`: `1724` lines, `87.12%` line / `82.73%` branch
  - `backend/src/session-manager.js`: `1121` lines, `95.09%` line / `75.00%` branch
  - `backend/src/runtime-startup-readiness.js`: `96.40%` line / `89.66%` branch
  - `backend/src/runtime-session-control-authority.js`: `445` lines, `99.55%` line / `93.87%` branch
...
- frontend:
  ...
  - `frontend/src/public/layout-profile-runtime-controller.js`: `631` lines, `93.32%` line / `85.90%` branch
  - `frontend/src/public/stream-interpretation-plugin-engine.js`: `212` lines, `95.75%` line / `90.22%` branch

Current active tasks after the second `H176` closeout:

- `QLT-323` Owner `BE`: isolate the next launch/reconnect/restore helper seam from `backend/src/session-manager.js`
- `QLT-324` Owner `FE`: extract the next initialization/reclaim/operator helper seam from `frontend/src/public/app-runtime-composition-controller.js`
