...
- `TODO.md`: the only active promoted task is now `QLT-249`, covering the retained operator-history and session-UI glue that still remain product-relevant after the two backend seam cuts, the frontend runtime-state seam extraction, and the operator layout/profile/settings plus operator command/workflow hardening in `v0.4.0-H159`.
...
- frontend: `96.53%` line / `88.80%` branch
...
The latest closeout validation on 2026-04-29 for `QLT-248` passed `node --test frontend/test/command-palette-runtime-controller.test.js frontend/test/custom-command-model.test.js frontend/test/slash-workflow-engine.test.js frontend/test/slash-workflow-runtime-controller.test.js frontend/test/slash-workflow-source-adapter.test.js`, `npm --prefix frontend run test:coverage`, `npm run docs:check`, `npm run lint`, `npm run test`, `npm run test:coverage:check`, and `git diff --check`. The validated coverage totals are root tooling `92.77%` line / `76.81%` branch, backend `94.64%` line / `87.82%` branch, and frontend `96.53%` line / `88.80%` branch.
