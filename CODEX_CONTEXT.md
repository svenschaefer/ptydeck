...
Last updated: 2026-05-05 (the backend runtime now also delegates session-control client/session authority into `backend/src/runtime-session-control-authority.js`, covering attachment lookup, state shaping, operator access checks, controller transfer/rename/forget helpers, scoped claimable-session resolution, and control-state broadcast/update orchestration directly; the backend runtime already delegates share/spectator request-access and runtime-metrics helper authority into `backend/src/runtime-access-policy.js` and `backend/src/runtime-metrics.js`, covering share-link admission, spectator route allowlisting, WS ticket extraction, and HTTP/WS metric helper mutation directly, now also delegates deck assignment, default deck creation, quick-id token lifecycle, unrestored-session lookup, and session-control model shaping into `backend/src/runtime-session-state.js`, now also delegates share-link, connection-profile, layout-profile, and workspace-preset authority into `backend/src/runtime-library-authority.js`, and still delegates SSH trust entry listing, trusted host-key type resolution, host-key probing, trust upsert/delete, conflict reuse detection, and managed `ssh_known_hosts` rendering into `backend/src/runtime-ssh-trust.js`; the backend session manager now also delegates timer/reconnect/pending-startup cleanup into `backend/src/session-manager-cleanup.js`, covering timer clearing, pending-launch cleanup, startup terminal-query fallback reset, and expected-exit cleanup directly, still delegates SSH remote-runtime reconnect/availability state shaping into `backend/src/session-manager-remote-runtime.js`, still delegates startup/post-start input lifecycle authority into `backend/src/session-manager-startup-runtime.js`, now also delegates app-identity refresh, terminal-signal observation, output-heuristic arbitration, and foreground-process refresh scheduling into `backend/src/session-manager-app-identity-runtime.js`, and still delegates SSH launch bundle assembly, reconnect scheduling/attempts, remote connectivity transitions, and PTY-exit lifecycle handling into `backend/src/session-manager-launch-runtime.js`; trusted-local handoff still resolves runtime client id through the session-control authority seam and fails closed on stale session takeover targets; SSH launches still pin `HostKeyAlgorithms` to the trusted host-key types for the selected target, pass an absolute managed `UserKnownHostsFile` path into spawned `ssh` processes, preserve canonical padded host-key base64 so the rendered managed `ssh_known_hosts` file stays parseable by OpenSSH, keep secret-backed auth on one masked action-dialog seam, expose slash-command host-key lifecycle management through `/ssh hostkey ...`, surface first-connect and rotation trust guidance directly in `Connections`, extend one-shot `/ssh ...` parity with `--deck`, `--cwd`, and `--command`, delegate session REST routing out of `backend/src/runtime.js` into `backend/src/runtime-session-dispatch.js`, delegate the remaining HTTP request entry seam out of `backend/src/runtime.js` into `backend/src/runtime-http-request-handler.js`, isolate restart payload shaping from `backend/src/session-manager.js` into `backend/src/session-manager-lifecycle.js`, extract trusted-local handoff/layout composition out of `frontend/src/public/app-runtime-composition-controller.js` into `frontend/src/public/app-runtime-trusted-local-composition.js`, extract runtime bootstrap-controller wiring out of `frontend/src/public/app-runtime-composition-controller.js` into `frontend/src/public/app-runtime-bootstrap-assembly.js`, extract runtime-event recovery wiring out of `frontend/src/public/app-runtime-composition-controller.js` into `frontend/src/public/app-runtime-recovery-composition.js`, extract session-control and quick-send runtime assembly out of `frontend/src/public/app-runtime-composition-controller.js` into `frontend/src/public/app-runtime-session-access-assembly.js`, extract workspace/send-history/trusted-local/paste-observation/broadcast-input operator-support assembly out of `frontend/src/public/app-runtime-composition-controller.js` into `frontend/src/public/app-runtime-operator-support-assembly.js`, now also extract startup/workflow/palette/initialization composition out of `frontend/src/public/app-runtime-composition-controller.js` into `frontend/src/public/app-runtime-startup-composition.js`, keep `frontend/test/app.test.js` resize and non-slash arrow-navigation assertions synchronized through deterministic waits instead of brittle fixed delays, extract SSH trust/launch lifecycle authority out of `frontend/src/public/connection-profile-runtime-controller.js` into `frontend/src/public/connection-profile-ssh-lifecycle.js`, extract connection-profile draft render and SSH trust UI state application out of `frontend/src/public/connection-profile-runtime-controller.js` into `frontend/src/public/connection-profile-ui-render.js`, extract `/settings` command dispatch out of `frontend/src/public/command-executor.js` into `frontend/src/public/command-executor-settings-handlers.js`, extract operator/discovery slash-command handling out of `frontend/src/public/command-executor.js` into `frontend/src/public/command-executor-operator-handlers.js`, extract retained replay/transfer command-reporting authority out of `frontend/src/public/command-executor.js` into `frontend/src/public/command-executor-reporting-handlers.js`, extract retained custom-command admin/discovery routing out of `frontend/src/public/command-executor.js` into `frontend/src/public/command-executor-custom-admin-handlers.js`, harden retained operator/session/replay router coverage across `frontend/src/public/command-executor.js`, `frontend/src/public/command-executor-operator-handlers.js`, and `frontend/src/public/command-executor-session-handlers.js`, import `formatConnectionProfileReport` directly from `frontend/src/public/connection-profile-draft-state.js` instead of duplicating that connection-profile reporting helper inside `command-executor.js`, extract connection-profile draft/profile/workspace state shaping out of `frontend/src/public/connection-profile-runtime-controller.js` into `frontend/src/public/connection-profile-draft-state.js`, extract computed connection-profile draft and SSH trust presentation out of `frontend/src/public/connection-profile-runtime-controller.js` into `frontend/src/public/connection-profile-runtime-view-state.js`, extract operator event binding out of `frontend/src/public/connection-profile-runtime-controller.js` into `frontend/src/public/connection-profile-ui-bindings.js`, and keep `v0.4.0-H172` active with `QLT-295`, `QLT-296`, `QLT-297`, and `QLT-298` completed plus `QLT-299` through `QLT-301` promoted)
...
- `TODO.md`: the active promoted tasks are `QLT-299`, `QLT-300`, and `QLT-301` inside `v0.4.0-H172`.
- `ROADMAP.md`: `v0.4.0-H172` is active with no queued next wave.
- `QLT-295`, `QLT-296`, `QLT-297`, and `QLT-298` are complete as the delivered slices inside `v0.4.0-H172`.
...
## Repo-Wide Quality Review Follow-Up (`v0.4.0-H172` active)

On 2026-05-05, the next quality-follow-up wave was promoted from the validated `H171` closeout tree and remains active after the first four delivered slices.
...
The latest validation on 2026-05-05 after `QLT-297` reran `npm --prefix backend run test:coverage`, `npm run docs:check`, `npm run lint`, `npm run test`, `npm run test:coverage:check`, and `git diff --check`. The validated coverage totals are root tooling `92.82%` line / `77.09%` branch, backend `95.81%` line / `88.95%` branch, and frontend `97.38%` line / `89.90%` branch.
