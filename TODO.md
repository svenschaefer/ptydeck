...
- [x] `QLT-119` Owner: `FE` Task: Extract the frontend runtime reducer/store into a dedicated module boundary with pure state transitions for sessions, decks, custom commands, connection state, filters, and related derived runtime metadata instead of continuing to mutate those domains inline in `app.js`.
