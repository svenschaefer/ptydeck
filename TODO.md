...
- [ ] `DOC-005` Owner `FE`: Add the first handbook frontend embedding as a static in-app documentation surface served under a dedicated handbook path instead of a new workspace panel, with a global `Handbook` entry point, stable deep links, a minimal table-of-contents shell, and context-link targets that can open the relevant handbook sections without duplicating handbook state inside the main runtime UI.
...
- [ ] `DOC-007` Owner `QA`: Add handbook drift checks and closeout validation for the first documentation wave, including generated-doc freshness enforcement, command/reference contract tests against current code, example-command verification for the documented slash-command flows, and smoke validation that the static in-app handbook entry and deep-link targets resolve correctly.
