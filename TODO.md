...
- `QLT-224` Owner `BE`: Add deeper direct runtime branch coverage and targeted hardening for `backend/src/messaging-runtime.js` and `backend/src/telegram-adapter.js`, covering distinct-path attention events, exact-repeat alerts outside suppression windows, inbound ambiguity and recovery, polling failure and backlog transitions, and other Telegram-path runtime branches that remain below the repo’s current quality bar.
- `APP-007` Owner `BE`: Harden local PTY active-app detection so coding-agent CLIs such as `codex` are identified from the full foreground process group instead of only a single representative process, preserving actionable wrapper and command-line evidence from shell, `sh -lc`, `node`, or similar launcher processes while still degrading deterministically for `tmux`, `screen`, SSH, and genuinely ambiguous sessions.
...
- `QLT-227` Owner `QA`: Add closeout validation for `v0.4.0-H84`, including focused regression coverage for the backend/frontend hotspot files promoted in `QLT-223`, `QLT-224`, `APP-007`, `QLT-225`, and `QLT-226` plus a deterministic full local `lint`, `test`, and `test:coverage:check` rerun on the final tree.
