...
- `MSG-016` Owner `BE`: Add an explicit maintained low-value messaging filter-pattern layer for agentic CLIs so ptydeck does not ship internal thought/process chatter such as `Ran ...`, `Edited ...`, inline diff/update summaries, long code-snippet echoes, or similar tool-internal progress records as user-facing Telegram messages, while keeping the patterns trace-backed and reviewable instead of hard-coding opaque one-off regexes.
- `MSG-015` Owner `QA`: Add closeout validation for `v0.4.0-H80`, including mocked Telegram duplicate/noise regression coverage, trace-log contract tests, low-value filter-pattern regression coverage for representative agentic CLI fragments, app-aware aggregation validation for coding-agent and generic-shell sessions, and a deterministic full local quality-gate rerun.
