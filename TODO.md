...
- [ ] `QLT-071` Owner: `FE` Task: Move command submit terminator configuration from global sidebar settings into per-terminal session settings so each terminal can use its own deterministic submit mode (`auto`/`CRLF`/`LF`/`CR`), including target-resolved routing behavior.
