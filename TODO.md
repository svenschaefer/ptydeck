...
2. `QLT-190` Owner `BE`
   Add focused backend regression coverage and hardening for `backend/src/proxy.js`, `backend/src/session-manager.js`, and `backend/src/ssh-host-key-probe.js`, including forwarded-header parsing edge cases, session attach or cleanup corner cases, reconnect-adjacent state handling, probe failure and empty-output behavior, and other branches that still rely too heavily on broad integration coverage instead of direct tests.
3. `QLT-191` Owner `FE`
