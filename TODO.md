...
2. `QLT-195` Owner `BE`
   Raise direct branch coverage and failure-path hardening for `backend/src/persistence.js` and the remaining under-exercised request and response branches in `backend/src/validation.js`, especially encrypted runtime-state save/load failures, rollback-envelope mismatches, and trusted-local control, ticket, and management-schema edge cases that still sit below the backend quality baseline after `H67`.
3. `QLT-196` Owner `FE`
   Add direct trusted-local identity and browser-state coverage plus hardening for `frontend/src/public/trusted-local-client-runtime-controller.js`, `frontend/src/public/startup-warmup-controller.js`, and the related `frontend/src/public/store.js` paths, covering malformed persisted device identity, unavailable browser storage, warmup fallback branches, stale local device labels, and multi-tab or reload state-reset behavior that still remains lightly exercised on the feature branch.
4. `QLT-197` Owner `FE`
