...
- Sessions support one optional persisted note; `/note` clears the active-session note, `@<sessionSelector> /note` clears another session's note, and a non-empty note is rendered in compact form inside the session header
