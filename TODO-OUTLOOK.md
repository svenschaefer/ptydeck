...
- [ ] `REM-010` Owner `FE`: Introduce a declarative completion-spec model for the ptydeck command plane (`/`, `>`, settings-related arguments) so command names, arguments, descriptions, and insertion behavior are data-driven instead of hardcoded across the frontend runtime.
- [ ] `REM-011` Owner `FE`: Add generator-backed contextual suggestion providers for command arguments (for example decks, sessions, tags, custom commands, paths, env keys, and themes) with deterministic caching, bounded latency, and explicit no-side-effect guarantees while typing.
- [ ] `REM-012` Owner `FE`: Add richer autocomplete presentation metadata (descriptions, examples, icons, completion kinds, and insert previews) while preserving deterministic keyboard-first behavior and fallback order.
- [ ] `REM-013` Owner `QA`: Add regression coverage for declarative/generator-backed autocomplete behavior, including deterministic ranking, timeout/fallback behavior, generator error isolation, and no unintended side effects during suggestion resolution.
