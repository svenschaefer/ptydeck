...
- [ ] `MSG-029` Owner `BE`: After the delivered `v0.4.0-H99` narrow codex-only first-use-case path, the delivered `v0.4.0-H100` shared replay/runtime evaluator parity, the delivered `v0.4.0-H103` block-identity delivery correction, the delivered `v0.4.0-H104` separator-tolerance widening, the delivered `v0.4.0-H105` chrome-stripped `codex_separator_section` follow-up, the delivered `v0.4.0-H106` strict `codex_separator_summary_sentence` follow-up, the delivered `v0.4.0-H107` duplicate/backoff plus delayed-submit correction, the delivered `v0.4.0-H108` Telegram slash-command parity wave, the delivered `v0.4.0-H109` restart-recovery summary suppression, the delivered `v0.4.0-H113` custom-command-only Telegram surface simplification, and the delivered `v0.4.0-H114` manual topic-title preservation have sufficient live-product evidence, continue the broader Telegram outbound rebuild toward an allowlist-/signal-first model with additional message-worthy state transitions, replayable trace-to-message evaluation, and deterministic block-aware new/update reuse before broader adapter/profile expansion resumes.
