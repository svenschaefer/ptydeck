...
6. the shipped narrow path now also prefers those neutral delivery signals for policy reasons, runtime trace identity, and summary restart-ledger keys, while the old `codex_*` scope names remain as compatibility metadata for historical analysis and restart-ledger continuity
7. while `legacy` remains the configured semantic primary mode, projection now stays genuinely non-authoritative in shipped runtime delivery, and the semantic-adapter layer explicitly rejects vertically fragmented transcript/diff leftovers plus footer metric chrome before those artifacts can become live `MessageIntent` text
...
After `MSG-083` through `MSG-088` plus the shipped post-baseline follow-ups through `H135`, ptydeck now has:
