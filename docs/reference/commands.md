...
| `/ssh` | start one-shot SSH sessions and manage SSH host-key trust | `/ssh <target>`<br>`/ssh <target> --key <path>`<br>`/ssh <target> --password`<br>`/ssh <target> --keyboard-interactive`<br>`/ssh <target> [-l|--user <username>] [-p|--port <port>] [--deck <deckSelector>] [--cwd <path>] [--command <command>]`<br>`/ssh hostkey list [target]`<br>`/ssh hostkey probe <target>`<br>`/ssh hostkey trust <target> [keyType|fingerprint]`<br>`/ssh hostkey delete <target> [keyType|fingerprint]` | - |
...
start one-shot SSH sessions and manage SSH host-key trust
...
| Subcommand | Description | Usage |
| --- | --- | --- |
| `/ssh hostkey` | manage SSH host-key trust for one-shot and saved SSH targets<br>Notes: The target syntax matches `/ssh`: `[user@]host[:port]`. Username is ignored for trust storage; trust is keyed by host, port, and host-key type. Run `/ssh hostkey probe <target>` first. If multiple fetched or trusted keys exist for the target, specify one by key type or fingerprint. | `/ssh hostkey list [target]`<br>`/ssh hostkey probe <target>`<br>`/ssh hostkey trust <target> [keyType|fingerprint]`<br>`/ssh hostkey delete <target> [keyType|fingerprint]` |
