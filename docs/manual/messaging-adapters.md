...
- `messaging.terminalMessagingCore.semanticAdapterIds`
...
- send the currently shipped narrow coding-agent outbound families, still named `codex_input_reply`, `codex_separator_info`, `codex_separator_section`, and `codex_separator_summary_sentence` for compatibility while generic outbound delivery remains hard-disabled; new block identities create new posts, only the same block identity is eligible for an edit, the restart-resend guard now covers all four shipped narrow families instead of only the summary family, the resend ledger now blocks both exact prior-history matches and coarse near-duplicate rewrites during restart recovery, larger coding-agent closing comments can now survive contaminated starts plus short transient noise long enough to emerge as one structured `codex_separator_section` message instead of fragmenting back into short `info` paragraphs or transient side signals, submitted-input reply promotion now rejects stale PTY carryover, repeated short-tail residue, and echoed operator input before the first real coding-agent answer line is delivered, commentary/progress chatter about repo/docs/runtime inspection work is suppressed across the narrow outbound families, ordinary operational status prose is no longer suppressed merely for mentioning debug logs, current runtime/code state, or explanatory `failed` wording inside structured bullets, turn replies now derive their primary semantic text from the shipped projection-backed `Turn` transcript/diff seam instead of the former first-hit line heuristic, short but correct replies no longer depend on the earlier minimum-length/minimum-word gate, autonomous coding-agent output can fall back to one projection-backed multiline section/info delivery at quiet completion, generic coding-agent adapters can normalize non-Codex markers such as Gemini-style `✦` through the same semantic registry seam, projection now drops standalone numeric fragment tails such as `46`, and unavoidable Telegram-visible truncation now preserves a continuous prefix with a final ellipsis instead of deleting an arbitrary middle block to keep the tail visible
- consume adapter-neutral `MessageIntent` delivery directly inside the Telegram adapter, applying per-thread policy decisions, session labeling, structured-text preservation, and bounded continuity-preserving truncation at the `DeliveryAdapter` seam instead of relying on runtime-owned Telegram formatting shortcuts
...
- all four families are Codex-only and block-aware:
  - `codex_input_reply` covers the non-separator case:
    ...
    - long reply lines are no longer clipped to the generic short summary budget before reply assembly, so a later family-level Telegram cap can still preserve one continuous visible prefix with a final ellipsis when bounded truncation is required
...
- the shipped narrow outbound families now also run behind one restart-recovery admission layer:
  - narrow delivery is suppressed before `runtime.ready`
  ...
  - a persisted resend ledger keyed by normalized content plus session/thread context prevents old already-delivered or trivially rewritten narrow outbound posts from reappearing on later restarts
...
- `/health.messaging.codexRestartRecovery` and `/ready.messaging.codexRestartRecovery` expose the current narrow-family restart-recovery state, including the configured quiet period, current post-ready quiet time remaining, active recovering-session count, and persisted resend-ledger size.
- `/health.messaging.codexSummaryRestartRecovery` and `/ready.messaging.codexSummaryRestartRecovery` remain as a backward-compatible alias for the same restart-recovery counters.
