...
- coding-agent `Session idle.` follow-ons are now also suppressed after recent low-value chatter or after a still-recent meaningful status update, so a filtered `Updated Plan`-style blip does not simply reappear as an idle-thread bump a few seconds later
