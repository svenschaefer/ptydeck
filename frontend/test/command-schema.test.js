...
test("command schema exposes declarative command metadata and distinct help/usage surfaces", () => {
  const schema = createSlashCommandSchema(["deck", "swap", "note", "layout", "workspace", "broadcast", "replay", "settings", "help", "run"]);
...
test("command schema formats command help text from declarative command summaries", () => {
  const helpText = createCommandHelpText(["new", "deck", "swap", "note", "layout", "workspace", "broadcast", "replay", "custom", "help", "run"]);
  assert.match(helpText, /^Commands: /);
  assert.equal(
    helpText,
    "Commands: @ > / new deck swap note layout workspace broadcast replay custom help run"
  );
});
