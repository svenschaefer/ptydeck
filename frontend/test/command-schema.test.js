...
test("command schema exposes declarative command metadata and distinct help/usage surfaces", () => {
  ...
  assert.equal(getSlashCommandUsage("replay"), "/replay export [selector|active] | /replay copy [selector|active]");
});
...
test("command schema formats command help text from declarative command summaries", () => {
  ...
  assert.match(helpText, /\/replay export \[selector\|active\] \| \/replay copy \[selector\|active\]/);
