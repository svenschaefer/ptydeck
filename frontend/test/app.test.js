...
test("app handles critical error paths, DOM lifecycle, and connection state rendering", async (t) => {
  ...
  assert.equal(secondSettingsPanelStartup.hidden, false);
  assert.equal(secondSettingsPanelNote.hidden, true);
  assert.equal(secondSettingsPanelTheme.hidden, true);
  secondSettingsTabNote.click();
  await tick();
  assert.equal(secondSettingsPanelStartup.hidden, true);
  assert.equal(secondSettingsPanelNote.hidden, false);
  assert.equal(secondSettingsPanelTheme.hidden, true);
  ...
  assert.equal(secondSettingsPanelStartup.hidden, true);
  assert.equal(secondSettingsPanelNote.hidden, true);
  assert.equal(secondSettingsPanelTheme.hidden, false);
