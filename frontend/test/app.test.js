...
test("app handles critical error paths, DOM lifecycle, and connection state rendering", async (t) => {
  ...
  await tick();
  assert.equal(fixture.elements.commandInput.value, "/connection");
  ...
  await waitFor(() => resizePayloads.some((entry) => entry.cols === 90 && entry.rows === 30), { timeoutMs: 1000 });
  assert.ok(
  ...
  await waitFor(
    () =>
      fixture.elements.commandFeedback.textContent ===
        "Terminal size set to 80x50 (cols x rows) for deck 'Default'." &&
  ...
  await waitFor(
    () =>
      fixture.elements.commandFeedback.textContent ===
        "Terminal size set to 90x50 (cols x rows) for deck 'Default'." &&
  ...
  await waitFor(
    () =>
      fixture.elements.commandFeedback.textContent ===
        "Terminal size set to 90x30 (cols x rows) for deck 'Default'." &&
      resizePayloads.some((entry) => entry.cols === 90 && entry.rows === 30),
    { timeoutMs: 1000 }
  );
  ...
  fixture.elements.sendCommand.click();
  await waitFor(
    () => fixture.elements.commandFeedback.textContent === "Terminal size set to 101x33 (cols x rows) for deck 'Ops'.",
    { timeoutMs: 1000 }
  );
  ...
  const secondSettingsTabStartup = secondSettingsPanel.querySelector(".session-settings-tab-startup");
  const secondSettingsTabNote = secondSettingsPanel.querySelector(".session-settings-tab-note");
  const secondSettingsTabTheme = secondSettingsPanel.querySelector(".session-settings-tab-theme");
  const secondSettingsPanelStartup = secondSettingsPanel.querySelector(".session-settings-panel-startup");
  const secondSettingsPanelNote = secondSettingsPanel.querySelector(".session-settings-panel-note");
  const secondSettingsPanelTheme = secondSettingsPanel.querySelector(".session-settings-panel-theme");
  const secondNoteInput = secondSettingsPanel.querySelector(".session-note-input");
  ...
  assert.equal(secondSettingsPanelStartup.classList.contains("session-settings-panel-active"), true);
  assert.equal(secondSettingsPanelNote.classList.contains("session-settings-panel-inactive"), true);
  assert.equal(secondSettingsPanelTheme.classList.contains("session-settings-panel-inactive"), true);
  secondSettingsTabNote.click();
  await tick();
  assert.equal(secondSettingsPanelStartup.classList.contains("session-settings-panel-inactive"), true);
  assert.equal(secondSettingsPanelNote.classList.contains("session-settings-panel-active"), true);
  assert.equal(secondSettingsPanelTheme.classList.contains("session-settings-panel-inactive"), true);
  assert.equal(secondSettingsTabNote.getAttribute("aria-selected"), "true");
  assert.equal(secondNoteInput.value, "");
  secondSettingsTabTheme.click();
  await tick();
  assert.equal(secondSettingsPanelStartup.classList.contains("session-settings-panel-inactive"), true);
  assert.equal(secondSettingsPanelNote.classList.contains("session-settings-panel-inactive"), true);
  assert.equal(secondSettingsPanelTheme.classList.contains("session-settings-panel-active"), true);
  assert.equal(secondSettingsTabTheme.getAttribute("aria-selected"), "true");
  secondSettingsTabStartup.click();
  await tick();
  assert.equal(secondSettingsPanelStartup.classList.contains("session-settings-panel-active"), true);
  assert.equal(secondSettingsPanelNote.classList.contains("session-settings-panel-inactive"), true);
  assert.equal(secondSettingsPanelTheme.classList.contains("session-settings-panel-inactive"), true);
