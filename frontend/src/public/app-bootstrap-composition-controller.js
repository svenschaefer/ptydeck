...
export function createAppBootstrapCompositionController(options = {}) {
  ...
  function composeControllers() {
    ...
    commandExecutor = createCommandExecutor({
      ...
      buildCustomCommandUsageApiOptions: (command) => options.buildCustomCommandUsageApiOptions?.(command),
      ...
      resolveWorkspaceGroup: (selectorText, deckId) =>
        workspacePresetRuntimeController?.resolveGroup?.(selectorText, deckId) || { group: null, error: "Unknown workspace group." },
      saveWorkspaceGroup: (name, deckId) => workspacePresetRuntimeController?.saveGroupByName?.(name, deckId) || "",
      applyWorkspaceGroup: (groupId, deckId) => workspacePresetRuntimeController?.applyGroupById?.(groupId, deckId) || "",
      renameWorkspaceGroup: (groupId, name, deckId) => workspacePresetRuntimeController?.renameGroupById?.(groupId, name, deckId) || "",
      deleteWorkspaceGroup: (groupId, deckId) => workspacePresetRuntimeController?.deleteGroupById?.(groupId, deckId) || "",
      clearWorkspaceGroup: (deckId) => workspacePresetRuntimeController?.clearGroupForDeck?.(deckId) || "",
    ...
    commandComposerRuntimeController = createCommandComposerRuntimeController({
      ...
      focusCommandGuardPrimaryAction: () => options.commandGuardSendOnceBtn?.focus?.(),
